...
`/auth/*`, the realtime `ws-ticket`, and read-only `…/search` and `…/query`
POSTs. Pass `--allow-mutations` to let everything through. All flags are
...
| `triage-queue.json` | Vision-model input, **layout findings only**. `shell`: the same finding (normalized selector and culprit) on 3+ routes, with one representative screenshot each. `items`: the remaining per-route findings at or above `--min-severity` (default medium), each with only the screenshots where they fired |
...
| `<app>/<route>/<viewport>-<theme>.png` | The renders |
...
1. **Bug triage:** give each `triage-queue.json` `shell` entry and `items`
   entry, plus its screenshots, to a cheap vision model (Haiku; Sonnet for
   precision), using `prompts/triage.md`. Batch 3–5 per call. Clean renders,
   axe findings and runtime errors never go to a model. On the first
   full run this was about 1 image in 8.
