...
| `PATCH` | `/orgs/organizations/order` | `partner` | Set the partner's preferred organisation display order. Body `{ "organizationOrder": ["<org-uuid>", ...] }`; unknown or deleted IDs are dropped, omitted orgs are appended in registration order. Persisted to partner settings and reflected by the list endpoints. Returns the sanitised `{ organizationOrder }`. |
