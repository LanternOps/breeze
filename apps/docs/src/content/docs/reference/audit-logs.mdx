...
**A fresh organization has no row in this table**, and the daily retention worker skips any organization without one — so audit logs accumulate indefinitely until a policy is set. Configure one from **Settings > Security & Access > Audit Retention** (requires the `audit:manage` permission, held by Org Admin by default), or directly via the API:
...
(There is no unique constraint on `org_id`, so check for an existing row first if you are not sure one hasn't already been created — `SELECT id FROM audit_retention_policies WHERE org_id = '<org-id>';`.)
