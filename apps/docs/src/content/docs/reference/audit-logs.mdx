...
**An organization created before that has no row**, and the daily retention worker skips any organization without one — so its audit logs accumulate indefinitely until someone sets a policy. This is the case for every organization on an upgraded install: nothing has been pruned and nothing will be until you save a policy, which also means nothing was quietly deleted by the upgrade. Configure one from **Settings > Security & Access > Audit Retention** (requires the `audit:manage` permission, held by Org Admin by default), or directly via the API:
