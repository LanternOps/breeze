...
When exceeded, the API returns `429` with a `Retry-After` derived from the sliding-window reset; agents respect this header and back off automatically, adding their own randomized jitter so a rejected fleet doesn't retry in lockstep.

Because the organization bucket is shared across every device in the tenant, low-frequency but high-value ingest (patch, inventory, hardware, software and event-log uploads — which run once per 15 minutes to once per 24 hours per device) is admitted from a small **reserved lane** worth 20% of the ceiling when the main bucket is exhausted. This prevents constant heartbeat traffic from starving the uploads that carry operator-facing device posture.
