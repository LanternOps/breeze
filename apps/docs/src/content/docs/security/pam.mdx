...
<Aside type="caution">
  **Human requester-vs-approver separation is not yet enforced.** The respond route does not compare the approving user against the requester, and the elevation flows that matter here (`uac_intercept`, `tech_jit_admin`) do not record an originating user identity at all (`subjectUserId` is `null` on every insert), so the control plane has nothing to compare against. Today, a user who holds `devices:execute` + MFA can approve a request -- including one tied to their own work. Treat requester/approver separation as an **operational** control: rely on approval reasons, the requester/device context surfaced in the UI, and anomaly alerting, not on a system-enforced block.
  ...
  Note that the live UAC-to-mobile fan-out is structurally a non-self-approval regardless: a pending `uac_intercept` elevation fans out one mobile approval request to each *eligible technician*, and those rows are created with `isRecursive: false` because the requester is an end user at the device, never the approving technician. The requester/approver identities differ by construction for that flow even though no system-enforced same-user block exists.
</Aside>
...
<Aside type="caution">
  Thumbprint pinning closes the CN look-alike gap, but it is opt-in per rule and per signer-group entry. A rule or group entry that carries only `matchSigner` / a bare CN string remains at the weak tier by the operator's own configuration -- Breeze does not retroactively upgrade existing CN-only rules. Security teams evaluating PAM should audit signer-based rules and prefer thumbprint pins for any rule carrying an auto-approve verdict.
</Aside>
...
- **CN look-alike signer spoofing.** Thumbprint-pinned rules and signer-group entries match the exact SHA-256 Authenticode certificate thumbprint rather than the self-asserted Common Name, so a forged or purchased certificate with a trusted-sounding CN cannot satisfy a thumbprint-pinned rule (#1776); the match fails closed rather than falling back to CN. See [Signer trust: CN matching vs. thumbprint pinning](#signer-trust-cn-matching-vs-thumbprint-pinning).
