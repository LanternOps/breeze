...
Cloudflare result via a strict allowlist map. The serial is gated on that **same** verified
condition — not just the verified flag — so an unverified or spoofed result can never carry a
real-looking serial number downstream. Full detail: `docs/operations/cloudflare-mtls-setup.md`
(including a spoofing-resistance test you can run against your own deployment).
