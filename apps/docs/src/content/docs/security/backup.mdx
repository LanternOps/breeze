...
Back up the volume separately from `scripts/backup.sh`:

```bash
docker run --rm -v breeze_api_data:/data -v "$(pwd)":/backup alpine \
...
Restore the archive with the stack stopped. Set `API_DATA_ARCHIVE` to the file you are restoring:

```bash
export API_DATA_ARCHIVE="api_data_YYYYMMDD.tar.gz"
docker compose down
docker run --rm -v breeze_api_data:/data -v "$(pwd)":/backup alpine \
  tar xzf "/backup/${API_DATA_ARCHIVE}" -C /data
docker compose up -d
```
...
Custom deployments must set `PATCH_REPORT_STORAGE_PATH` to a persistent location. Its default is the relative path `./data/patch-reports`, which resolves to `/app/data/patch-reports` in the API container and is lost when that container is replaced. The shipped Compose files set `/data/patch-reports` so the files use `api_data`.
