...
description: Track backup health, job history, storage trends, and per-device protection status.
sidebar:
  order: 4
...
- Status: **Completed** (green), **Running** (blue), **Failed** (red), **Queued** (gray), **Canceled**, **Stalled**
- Start time, duration, and data size

While a job is **Running**, the card shows live progress — files transferred so far and the current transfer speed — updated as the agent reports in. If the agent stops reporting progress mid-run, the card shows a **Stalled** badge so you can spot a wedged job before it is auto-failed.
...
| **Stalled** | Running, but the agent has stopped reporting progress. Shown as a warning badge; if progress does not resume, the job is auto-failed (see below) |
...
### Automatic failure of stuck jobs

A job never stays **Running** forever. Breeze marks a job **Failed** with an explanatory error — rather than leaving it stuck — when any of these happen:

- **No progress for 15 minutes** — an agent that was reporting progress goes silent.
- **The device goes offline mid-run** — after a short grace period to cover a reboot.
- **A 24-hour ceiling** — a hard upper bound for older agents that don't report incremental progress.

The recorded error explains which condition tripped, so the job history stays accurate instead of showing a perpetual **Running**. If a genuine success report arrives late — for example, the job actually finished right as it was being failed — Breeze reconciles the job back to **Completed** so its snapshot isn't stranded.
