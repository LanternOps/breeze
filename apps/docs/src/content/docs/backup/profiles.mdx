...
When you create a profile, the first two templates in the list are **Whole machine (Linux)** and **Whole machine (Windows)**. Each turns on both the **Files** and **System State** sources so a single snapshot carries everything [bare-metal recovery](/backup/bare-metal-recovery/) needs — the whole root filesystem (`/` or `C:\`), the OS state, and the disk layout — and seeds a root-anchored exclude list for the paths that must never be backed up:
