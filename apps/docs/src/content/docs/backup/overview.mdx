...
<CardGrid>
  <Card title="Files & Folders" icon="document">
    Back up selected directories with include/exclude path rules. Uses Volume Shadow Copy (VSS) on Windows for application-consistent snapshots. Incremental after the first run — only changed files are uploaded.
  </Card>
