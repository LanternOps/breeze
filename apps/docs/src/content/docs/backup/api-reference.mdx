...
| GET | `/backup/restore/:id` | Get restore status and progress (0–100) |
...
| POST | `/backup/instant-boot` | Instant boot a VM from snapshot |
...
| Method | Path | Description |
|--------|------|-------------|
| POST | `/backup/verify` | Run verification (`deviceId`, `verificationType`: integrity/test_restore/full_recovery) |
