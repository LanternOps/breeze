...
| POST | `/backup/mssql/restore` | Restore a database from a backup snapshot (`deviceId`, `snapshotId`, `targetDatabase`, optional `instance`, optional `noRecovery`). Returns `202` with `commandId` and `restoreJobId` once the restore is dispatched — see note below |

<Aside type="note" title="SQL Server and Hyper-V restores are asynchronous (v0.117.0+)">
  `POST /backup/mssql/restore` and `POST /backup/hyperv/restore` return `202 Accepted` as soon as the restore command is queued, instead of holding the request open for up to 10 minutes and reporting a failure while the agent was still restoring. The response carries a `restoreJobId`; poll `GET /backup/restore/:id` for the terminal status. A device that is offline is rejected with `409`.
</Aside>
...
| POST | `/backup/hyperv/restore` | Import a VM from a backup snapshot (`deviceId`, `snapshotId`, optional `vmName`, optional `generateNewId`, default `true`). Returns `202` with `commandId` and `restoreJobId` once dispatched |
