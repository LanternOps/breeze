...
```yaml
# agent.yaml (0640 — readable by Breeze Assist helper)
server_url: "https://breeze.yourdomain.com"
...
<Aside type="danger">
  The `secrets.yaml` file contains the agent's bearer token and is restricted to `0600` (owner-only read/write). The `auth_token` is intentionally duplicated in `agent.yaml` (which uses `0640` permissions) so the Breeze Assist helper process can discover the token without needing root access to `secrets.yaml`. Do not change these permissions.
</Aside>
