...
| `deadline` | string | `now + delayMinutes` | RFC 3339 hard stop. The API always sends it; a payload without it falls back to the calculated reboot time |
| `allowDeferral` | bool | `false` | Whether the signed-in user may postpone this restart. Absent means `false` -- a restart is never postponable by default |
| `maxDeferrals` | int | `0` | How many postponements this restart allows |
| `deferralMinutes` | int | `0` | How far each postponement pushes the restart out |
