Looking at the code changes, I need to analyze what was modified:

1. **service_windows.go**: Added SCM session change event handling to detect user session changes (login/logout/disconnect)
2. **handlers_desktop.go**: Improved session detection to prefer active sessions but fall back to connected sessions (for lock screen scenarios after reboot)
3. **heartbeat.go**: 
   - Added `scmSessionCh` channel for SCM session events
   - Added `sendDesktopDisconnectNotification()` method to notify API when WebRTC peer drops
   - Added `SCMSessionCh()` getter method
   - Added helper lifecycle manager initialization in `Start()`
   - Added `OnSessionStopped` callback for direct mode (non-service)
   - Added handling of `TypeDesktopPeerDisconnected` IPC message

The key functional changes are:
- Remote desktop quality pulsing is reduced by proactively spawning helpers into user sessions after reboot (Windows service mode)
- Desktop disconnect notifications are now sent to the API when WebRTC peers disconnect
- Better session detection after reboot (connected sessions fallback)

The documentation needs updates in the Remote Desktop section to reflect the new desktop disconnect notification behavior and session management improvements.

---

Here's the updated documentation file:
...
Two remote desktop modes are available: WebRTC-based (legacy) and WebSocket-based streaming. The agent automatically manages helper processes in user sessions and notifies the API when peer connections drop.

<Aside>
On Windows service deployments, the agent proactively spawns helpers into user sessions upon startup and session changes, ensuring remote desktop is instantly available after reboot. This reduces connection latency and eliminates "quality pulsing" caused by lazy helper initialization.
</Aside>
...
When a WebRTC peer connection drops, the agent automatically sends a disconnect notification to the API containing:

| Field | Type | Description |
|---|---|---|
| `type` | string | `"command_result"` |
| `commandID` | string | `"desk-disconnect-{sessionId}"` |
| `status` | string | `"completed"` |
| `result.sessionId` | string | Session identifier |
| `result.event` | string | `"peer_disconnected"` |

This allows the API to mark the session as disconnected and enable the viewer to reconnect. The notification is sent:
- In direct mode (non-Windows service) when the local peer connection ends
- In Windows service mode when the user-mode helper detects the peer disconnect via IPC
