...
```
C:\Program Files\Breeze Assist\Breeze Assist.exe
```
...
When installed via config policy, the LaunchAgent plist is at `/Library/LaunchAgents/com.breeze.assist.plist`. When using the built-in `user-helper` subcommand, the plist is at `/Library/LaunchAgents/com.breeze.agent-user.plist`. Both start automatically when the user logs in.
