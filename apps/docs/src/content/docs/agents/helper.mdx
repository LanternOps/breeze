---
title: Breeze Helper
description: System tray application providing end-user AI chat, vision-based troubleshooting, and self-service tools.
sidebar:
  order: 5
  label: Breeze Helper
---
...
<Aside>
  The Helper requires the Breeze agent to be enrolled and connected. It authenticates to the API using the agent's `brz_` token and communicates with the agent service over a local IPC socket.
</Aside>
...
The Helper is installed automatically as part of the Breeze agent deployment. On Windows, the agent service (Session 0) spawns the Helper into the user's desktop session via the session broker. On Linux and macOS, it runs as a user-level service.
...
The Helper connects to the agent service via a Unix domain socket at `/var/run/breeze/helper.sock`.
...
The LaunchAgent plist is installed to `~/Library/LaunchAgents/com.breeze.helper.plist` and starts automatically when the user logs in.
...
<Aside type="caution">
  The **extended** permission level grants access to `execute_command`, `security_scan`, and `network_discovery`. These are Tier 3 tools that require explicit approval before execution. Only enable extended mode for trusted environments where end users should be able to trigger device commands.
</Aside>
...
<Aside type="caution">
  Computer control is a powerful capability. The AI always explains what it intends to do before acting, and destructive operations require user approval. The `computer_control` tool is not available at the `basic` permission level.
</Aside>
...
<Aside>
  All Helper endpoints require a valid agent bearer token (`Authorization: Bearer brz_...`). The token is validated by hashing with SHA-256 and matching against the `agentTokenHash` column in the `devices` table. Decommissioned and quarantined devices are rejected with 403.
</Aside>
