...
<Aside type="caution">
  The `authToken` is only returned once. The agent stores it in its local config file (`/etc/breeze/config.json` on Linux/macOS, `C:\ProgramData\Breeze\config.json` on Windows) with restrictive file permissions (0600). The API stores only the SHA-256 hash.
</Aside>
