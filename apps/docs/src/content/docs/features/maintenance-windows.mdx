...
Exit is not gated by a second factor -- ending suppression early is never the risky direction. On exit the device's status is **recomputed from when its agent was last seen** rather than stamped back to `online`, so a device that dropped off the network during the window returns as `offline` instead of being resurrected as online.
...
Breeze issues a scheduled reboot with a 15-minute grace period. The signed-in user gets a "save your work" warning about 5 minutes before the reboot fires.
</TabItem>
<TabItem label="Linux">
Breeze schedules the reboot 15 minutes out (equivalent to running `shutdown -r +15`), which also warns any logged-in users.
</TabItem>
<TabItem label="macOS">
