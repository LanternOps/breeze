...
The asset modal includes a **Proxy Access** section that lets you open a LAN device's built-in web interface (router admin page, NAS panel, printer UI, switch management port, etc.) directly inside Breeze — no VPN required.

<Steps>

1. Open the asset modal by clicking any discovered asset in the list.
...
Traffic is tunneled over the bridge agent's existing WebSocket connection to Breeze — no additional firewall rules or port forwarding required. See [Network Proxy](/features/remote-access/#network-proxy) in the Remote Access docs for allowlist configuration and the full security model.
