...
When the fallback activates, the system tunnels VNC through the agent connection and renders it in the browser using noVNC. When you disconnect, the VNC password is cleared — no credentials persist on the device.
