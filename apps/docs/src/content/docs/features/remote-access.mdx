...
- **Firewall blocking TURN ports** — Coturn needs TCP+UDP port 3478, plus TCP+UDP ports in the relay range (default 49152–65535, or tightened to 49152–49252). TCP is required because Breeze advertises `transport=tcp` TURN URLs for clients behind restrictive firewalls. Verify with `nc -z -w 3 TURN_IP 3478` (TCP) and `nc -u -z -w 3 TURN_IP 3478` (UDP).
