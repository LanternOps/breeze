...
All remote access runs over WebRTC. The Breeze server acts as a signaling relay during connection setup — it brokers the initial SDP offer/answer exchange and ICE candidate negotiation between the browser and the device agent. Once the peer-to-peer connection is established, device data flows directly between the browser and the agent and does not transit the Breeze server. Terminal output, screen pixels, and file contents are never stored or buffered server-side.
...
The maximum file size per transfer is **5 GB**. Transfers exceeding this limit are rejected at creation time.
...
When a session ends (`disconnected` or `failed`), the platform records `durationSeconds` (total wall-clock time active) and `bytesTransferred` (total data exchanged) for audit purposes.
...
**Server is a signaling relay only.** The Breeze server brokers WebRTC negotiation (SDP offer/answer and ICE candidates) but does not proxy or store device data after the peer-to-peer connection is established.
...
WebRTC negotiation failed. Confirm the device is online and the agent WebSocket connection is active (check the device detail page). ICE negotiation fails when both endpoints are behind symmetric NAT and no TURN server is configured — contact your administrator to verify TURN configuration.
