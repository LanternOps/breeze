...
Older macOS devices running Monterey or Ventura do not support WebRTC remote desktop at the login screen. Breeze auto-detects this limitation and falls back to VNC via the built-in macOS Screen Sharing service.

When the fallback activates, the system enables Screen Sharing with a one-time ephemeral password, tunnels VNC through the agent connection, and renders it in the browser using noVNC. When you disconnect, Screen Sharing is automatically disabled and the password is cleared — no credentials persist on the device.

This works in both the browser and the Breeze viewer app. No manual configuration is needed — the **Connect Desktop** button auto-detects the situation and displays a VNC badge on the session.
