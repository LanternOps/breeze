...
```
draft → pending → running → downloading/installing → completed
                ↓                                  → failed
              paused                               → cancelled
              (resume → running)                   → rollback
```
...
| `running` | Execution is active; devices in the current batch are being dispatched. |
...
Begins execution. For an immediate deployment, all device commands are dispatched at once. For a staggered deployment, the first batch is dispatched and the engine waits for batch completion (or the `batchDelayMinutes` timer) before starting the next batch.

The deployment status moves from `pending` to `downloading` or `installing` as devices begin processing.
...
| `POST` | `/deployments/:id/pause` | Pause between batches (staggered deployments only). |
