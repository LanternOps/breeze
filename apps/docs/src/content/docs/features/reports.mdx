...
<Aside type="note">
  You can start this report straight from the report builder by choosing the **Security & Compliance Posture (Insurance)** template card. The direct-create modal asks only whether backup coverage is required; the report name, date range, schedule, and output format use the template defaults. Once created, it generates and downloads exactly like the rest of your reports (it appears in your reports list as "Security & Compliance Posture").
