...
<Aside type="note">
  You can start this report straight from the report builder: pick the **Security & Compliance Posture (Insurance)** template card, then set the name, date-range preset, schedule, and output format like any other report. Once saved, it generates and downloads exactly like the rest of your reports (it appears in your reports list as "Security & Compliance Posture").
