...
### AI product copy style

Two AI buttons write catalog copy for you, and both follow a house style you control:

- **Auto-fill from web** drafts a whole item (name, description, type, unit of measure, taxability, and a price estimate) from a product name or SKU.
- **Polish** cleans up the wording of a name or description you already have — fixing grammar, capitalization, and formatting — without changing any numbers, measurements, or model numbers. It shows a before/after preview you approve before anything changes.

To shape how these read, set an **AI product copy style** in **Settings → Billing settings**. It's a free-text description of your preferred house style (there are no fixed presets) and applies partner-wide — to catalog items, quotes, and invoices. Leave it blank to use Breeze's built-in default format (a short customer-friendly name, then a specs description). The style only influences wording and formatting; it never overrides factual accuracy.
...
This is distinct from the catalog-level import workflow documented above. The catalog workflow builds your standing price list; the quote-level search lets you pull a product directly into an open quote without navigating away from the quote editor.

### Inline editing of quote line items

Quote line items can now be edited in place directly in the quote editor — you no longer need to delete a line and re-add it to correct a detail. Click any field on an existing line item to edit it inline:
