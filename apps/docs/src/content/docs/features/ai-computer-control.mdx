...
<Aside type="note">
  Three of these tools give the AI direct visual and input control over a device's desktop, each Tier 3 (human approval required): `take_screenshot` captures the device screen for visual analysis; `analyze_screen` captures a screenshot and analyzes what's visible against a context you describe (for example, "error dialogs" or "performance issues"); `computer_control` sends mouse and keyboard input (clicks, scrolling, typing) and returns a screenshot after each action. This page's tool table below does not yet cover these three -- treat their presence here as a placeholder pending a fuller pass.
</Aside>
...
| `execute_command` | List processes/files/event-log channels (Tier 2); kill/service control/file read/service list/event-log query (Tier 3) | Tier 2/3 | Tier 3 command types only |
...
| `file_operations` | List directories (Tier 2); read/write/delete/mkdir/rename (Tier 3) | Tier 2/3 | Read/write/delete/mkdir/rename |
...
<Aside>
  Tier classification is determined at the **action level**, not the tool level. For example, `file_operations` is Tier 2 when listing a directory, but escalates to Tier 3 when reading, writing, or deleting files. The AI resolves the effective tier at runtime for each specific invocation. See [AI Features](/features/ai/) for full governance details.
</Aside>
...
`run_script` is Tier 3 — every invocation requires human approval before it reaches the device. `execute_command` is split per command type (see below): most command types are Tier 3 and require approval; `list_processes`, `file_list`, and `event_logs_list` are Tier 2 and run without a durable approval step in auto-execute session modes (they're still audit logged, and under the default per-step mode they still take a lightweight inline confirmation).
...
| Command Type | What It Does | Tier |
|--------------|--------------|------|
| `list_processes` | List running processes | 2 |
| `kill_process` | Terminate a process by ID or name | 3 |
| `list_services` | List system services and their status | 3 |
| `start_service` | Start a stopped service | 3 |
| `stop_service` | Stop a running service | 3 |
| `restart_service` | Restart a service | 3 |
| `file_list` | List files in a directory | 2 |
| `file_read` | Read file contents | 3 |
| `event_logs_list` | List available event log channels | 2 |
| `event_logs_query` | Query event log entries with filters | 3 |

`list_services` and `event_logs_query` stay Tier 3 even though they're reads: service records can carry credentials in their binary path/arguments, and event-log entries (particularly the Security log) can carry a mistyped password or other sensitive text in their message body.

The device must be online (WebSocket connected) to receive commands. Commands have a 120-second timeout.
...
<Aside type="caution">
  When the AI proposes a Tier 3 `execute_command` action, you will see a detailed description of the command in the approval prompt -- for example, "Execute `kill_process` command on device a1b2c3d4...". Review the command type and payload before approving. Tier 2 command types (`list_processes`, `file_list`, `event_logs_list`) skip this durable approval step in auto-execute session modes.
</Aside>

### run_script

Executes a script from the Breeze script library on one or more devices (up to 10 per invocation). Scripts are referenced by their library UUID, not inline code. Each device receives the script independently with a 120-second timeout.
...
Performs file system operations on a device. Listing a directory auto-executes (with an audit record); everything that touches file *contents* — including reads — requires approval, because the agent accesses the filesystem with system-level privileges.

| Action | Tier | Approval | Description |
|--------|------|----------|-------------|
| `list` | Tier 2 | No (audit logged) | List directory contents |
| `read` | Tier 3 | **Yes** | Read file contents |
...
The device must be online for all file operations. Commands have a 60-second timeout.
...
Scans have a 120-second timeout. Threat management actions (`quarantine`, `remove`, `restore`) require the `threatId` from a previous scan or status check.
...
| `file_operations` | all | `devices:execute` |
...
Device commands have a 60-second default timeout (120 seconds for `execute_command`, scripts, security scans, network discovery, screenshots, and computer control). If a device is under heavy load or has a slow network connection, commands may time out. Verify the device is online and responsive, then retry.
