...
| `valueType` | Data type: `number`, `string`, `null`, `object`, or `hex` |
| `timestamp` | ISO 8601 collection timestamp |

#### The `hex` value type

Some devices answer certain OIDs with a raw **binary** octet-string rather than
text — the bridge and forwarding-database OIDs are the common case
(`.1.3.6.1.2.1.17.1.1.0` `dot1dBaseBridgeAddress`,
`.1.3.6.1.2.1.17.4.3.1.1.*`), where the "string" is really a six-byte MAC
address. Those bytes cannot be stored in a text column: a NUL byte is rejected
outright by PostgreSQL, and other byte sequences would be silently mangled.

When that happens the reading is stored as **lowercase, separator-free hex** and
`valueType` is set to `hex`. A MAC of `78:8a:20:00:d4:e1` is stored as the value
`788a2000d4e1`.

**Always check `valueType` before doing anything with a value, and never parse a
`hex` value as a number.** Hex digits can come out entirely numeric
(`001122304050`), so a naive numeric parse turns a MAC address into a nonsense
reading of ~1.1e11. Breeze's own metric rollups and the dashboard's
top-interfaces ranking exclude `hex` rows for exactly this reason.

How faithful the hex is depends on which side encoded it:

- **Encoded by the agent** — current agents detect these values and flag the
  encoding on the wire. This path is lossless: decode the hex back to bytes and
  you get exactly what the device sent, as in the `788a2000d4e1` example above.
- **Encoded by the API as a fallback** — used for readings from older agents
  that don't send that flag. Here the damage is already done before Breeze sees
  the value: the older agent packs the reading into JSON first, and that step
  replaces every byte that isn't valid text with the Unicode replacement
  character. The API can only hex-encode what actually arrived, so those
  replacements are what get stored. In practice you'll see runs of `efbfbd`
  (the replacement character) where the device's real bytes used to be.

So treat a `hex` value from the fallback path as a **stable identifier** for
that reading — fine for matching, grouping, or spotting a change — but not as a
faithful dump of the device's bytes. If you need the true bytes off a device,
upgrade its agent so the agent-side encoding applies.
