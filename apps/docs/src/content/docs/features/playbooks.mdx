...
Breeze includes three built-in playbooks that are seeded into the database on startup. Built-in playbooks have `isBuiltIn: true` and a `null` organization ID, making them available to all organizations. They are updated automatically when the API starts — if a built-in playbook's definition changes in a new release, the existing database record is updated via upsert.
...
Custom playbooks are created by inserting into the `playbook_definitions` table through the API or directly via the database. Each playbook definition includes:
...
Every playbook run creates an execution record in the `playbook_executions` table. Execution records include:
