...
- **Linked** — points to an existing policy object by ID (e.g., an existing update ring). Changes to the linked object propagate automatically. Not available for `monitoring` — use inline instead.
...
When calling the features endpoints, the `featureType` value must be one of: `patch`, `alert_rule`, `backup`, `security`, `monitoring`, `monitors`, `maintenance`, `compliance`, `automation`, `event_log`, `software_policy`, `sensitive_data`, `peripheral_control`, `warranty`, `helper`, `remote_access`, `pam`, `onedrive_helper`, `vulnerability`, `device_lifecycle`. `monitors` is the authoring/deployment feature. `alert_rule` and `monitoring` remain accepted only for transitional compatibility during W05c; use the conversion panel for their existing rows.
