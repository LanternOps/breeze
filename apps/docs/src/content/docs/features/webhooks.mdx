...
| `url` | text | Destination URL (must be HTTPS); encrypted at rest using AES. GET responses strip any credentials embedded in the URL (HTTP basic-auth userinfo and signed query tokens) before returning it. |
