...
<Aside type="tip">
  The software catalog, deployments, and inventory endpoints are fully persisted via Drizzle ORM with PostgreSQL. Software packages can be uploaded to S3 with SHA-256 checksum verification and deployed silently to endpoints. The sidebar labels for these features are **App Library** (`/software`) and **App Policies** (`/software-inventory`).
</Aside>
