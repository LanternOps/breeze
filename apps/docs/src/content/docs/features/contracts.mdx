...
A contract line scoped to a site records the site's name as well as its id. If that site is deleted, the line **stops billing and says so**. The contract detail shows "site deleted," and generating an invoice refuses with a named error, instead of quietly falling back to billing every device in the organization. Re-scope the line to another site, or remove it, and generation resumes.
