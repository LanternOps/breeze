...
1. If the scan targets a subdirectory rather than a listed volume root, a `baseline` scan is always used.

2. If a previous scan was interrupted and checkpoint data exists, a `baseline` scan resumes from the checkpoint.

3. If no baseline has ever been completed for this volume, a `baseline` scan is started.
...
|-------|------|-------------|
| `deviceId` | UUID | Device identifier. Together with `scanPath`, identifies one state record per volume. |
| `scanPath` | text | Normalised volume path. Nullable during the rolling-upgrade window. |
...
| `scan_path` | text | The normalised volume/path this scan covered. One snapshot stream per volume. Nullable during the rolling-upgrade window. |
...
| Column | Type | Description |
|--------|------|-------------|
| `device_id` | UUID | Foreign key to `devices`. First half of the unique key. |
| `scan_path` | text | The normalised volume. Second half of the unique key -- one checkpoint and baseline per volume. Nullable during the rolling-upgrade window. |
| `scan_generation` | UUID | Command ID owning the current scan. Claimed when applying results to reject superseded or duplicate results. |
