...
Backup configurations support five storage provider types:
...
| `local` | Local filesystem, network-attached storage, or USB drives. Configuration details include the mount path, retention days, and compression algorithm. |
| `azure_blob` | Azure Blob Storage. Configuration details include SAS tokens and connection strings. |
| `google_cloud` | Google Cloud Storage. Configuration details include service account keys. |
| `backblaze` | Backblaze B2 cloud storage. Configuration details include B2 API keys. |
...
## Backup Scheduling (Configuration Policies)

Backup scheduling and retention are managed through [Configuration Policies](/features/configuration-policies/). Assign a policy with a **Backup** feature to any scope (organization, site, device group, or device) and the scheduler resolves which devices need backups using the same hierarchical precedence rules as other feature types.

<Aside type="tip">
  The standalone `backup_policies` table is deprecated. Existing policies continue to work, but new backup schedules should be created through Configuration Policies for hierarchy-based device resolution and enterprise backup mode support. See [Configuration Policies — Backup](/features/configuration-policies/#backup) for details.
</Aside>

### Backup Modes

Configuration Policies support four backup modes:

| Mode | Description |
|------|-------------|
| `file` | File-level backup of selected directories (default) |
| `hyperv` | Hyper-V VM export with application or crash consistency |
| `mssql` | SQL Server database backup (full, differential, and log) |
| `system_image` | Full system image capture |

Hyper-V and MSSQL modes use **all-by-default targeting** — newly discovered VMs and databases are automatically included on the next scheduled run. You can optionally configure exclude lists to skip specific VMs or databases.

### Binary Architecture

Enterprise backup operations (Hyper-V, MSSQL, system image, C2C) run in a dedicated `breeze-backup` process that communicates with the main agent over HMAC-signed IPC. This keeps the core agent lightweight (~15 MB) while the backup binary includes the heavier cloud SDKs and backup tooling. The backup binary is bundled in both the Windows MSI and macOS .pkg installers.
...
The following features extend the core backup system with application-aware, platform-specific, and compliance-oriented capabilities.

### Volume Shadow Copy (VSS)

On Windows devices, Breeze uses Volume Shadow Copy Service to create application-consistent snapshots. VSS ensures that databases, email servers, and other write-heavy applications are in a clean state before the snapshot is taken.

- Automatic volume detection from backup paths
- VSS writer status monitoring (SQL Server, Exchange, Hyper-V writers)
- Configurable timeout (default 600 seconds)
- Fallback to file-level backup if VSS is unavailable

Check VSS writer health for a device:

```bash
GET /backup/vss/status/:deviceId
```

Returns a list of VSS writers and their current states (`stable`, `failed`, `waiting`, `unknown`).

### SQL Server Backup (MSSQL)

Breeze can discover, back up, and restore SQL Server databases directly — no third-party SQL backup tool required.

**Discovery:**

```bash
GET /backup/mssql/instances
GET /backup/mssql/instances/:deviceId
```

Returns discovered SQL Server instances with name, version, edition, authentication type, and status. Each instance includes its databases with size, recovery model, TDE status, and compatibility level.

**Backup:**

```bash
POST /backup/mssql/backup
```

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `deviceId` | UUID | Yes | Target device |
| `instance` | string | Yes | SQL Server instance name |
| `database` | string | Yes | Database to back up |
| `backupType` | string | Yes | `full`, `differential`, or `log` |
| `outputPath` | string | Yes | Backup file destination |
| `configId` | UUID | No | Backup configuration for cloud upload |

Differential and log backups track LSN chains for point-in-time recovery.

**Restore:**

```bash
POST /backup/mssql/restore
```
...
| `deviceId` | UUID | Yes | Target device |
| `instance` | string | Yes | SQL Server instance name |
| `backupFile` | string | Yes | Path to the backup file |
| `targetDatabase` | string | Yes | Database name to restore into |
| `noRecovery` | boolean | No | Leave database in restoring state for log chain application |

### Hyper-V VM Backup

Back up, restore, and manage Hyper-V virtual machines including checkpoints and power state control.

**VM Discovery:**

```bash
GET /backup/hyperv/vms
GET /backup/hyperv/vms/:deviceId
```

Returns VMs with name, generation, memory, CPU count, VHD paths, and current state. Filter by `deviceId` or `state`.

**VM Backup:**

```bash
POST /backup/hyperv/backup
```

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `deviceId` | UUID | Yes | Hyper-V host device |
| `vmName` | string | Yes | VM to back up |
| `exportPath` | string | Yes | Export destination |
| `consistencyType` | string | No | `application` (default, uses Hyper-V VSS) or `crash` (saves state first) |

Supports full and incremental backups via Resilient Change Tracking (RCT).

**VM Restore:**

```bash
POST /backup/hyperv/restore
```

Imports an exported VM to a host, generating a new VM ID. Also supports checkpoint management (`POST /backup/hyperv/checkpoint`) and VM power state control (`POST /backup/hyperv/vm-state`).

### Bare Metal Recovery (BMR)

Full system recovery from backup to bare hardware or a replacement device.

**Recovery Token:**

```bash
POST /backup/bmr/token
```

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `snapshotId` | UUID | Yes | Snapshot to recover from |
| `expiresInHours` | number | No | Token lifetime (default varies) |
| `restoreType` | string | No | `full`, `selective`, or `bare_metal` |
| `targetConfig` | object | No | Target hardware configuration |

Returns a one-time recovery token (shown once, stored as SHA-256 hash). Recovery agents authenticate with this token instead of JWT — no pre-existing enrollment required.

The BMR process:
1. Boot recovery media on target hardware
2. Authenticate with the recovery token
3. Download and restore system state (OS config, drivers, registry on Windows; /etc, boot config on Linux)
4. Restore all backed-up files
5. Post-restore validation (service status, network connectivity, critical file presence)

### Backup Verification

Verify that backups are recoverable before you need them.
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `deviceId` | UUID | Yes | Device whose backup to verify |
| `verificationType` | string | Yes | `integrity`, `test_restore`, or `full_recovery` |
| `backupJobId` | UUID | No | Specific job to verify |
| `snapshotId` | UUID | No | Specific snapshot to verify |
| `highImpactApproved` | boolean | No | Required for `full_recovery` verification |

**Verification types:**

| Type | Duration | What it checks |
|------|----------|----------------|
| `integrity` | Fast | Checksums, file counts, manifest consistency — no restore required |
| `test_restore` | Medium | Extracts entire snapshot to temp location, measures restore time (RTO) |
| `full_recovery` | Long | Full system restore to target device with post-restore validation (requires approval) |

List verification results:

```bash
GET /backup/verifications?deviceId=&verificationType=&status=&from=&to=&limit=
```

### Recovery Readiness

Track how prepared each device is for recovery:

```bash
GET /backup/recovery-readiness?deviceId=&refresh=true
```

Returns a readiness score (0–100) per device based on:
- How recent the latest snapshot is
- Whether recent verifications passed
- Estimated RTO and RPO
- Risk factors (missing backups, failed verifications, expired snapshots)

Organization-level backup health:

```bash
GET /backup/health?refresh=true
```

Returns overall status (`critical`, `degraded`, `healthy`), escalation counts, and readiness metrics.

### Encryption Key Management

```bash
GET /backup/encryption/keys
POST /backup/encryption/keys
GET /backup/encryption/keys/:id
```

Manage AES-256 encryption keys for backup snapshots. Keys are tracked with SHA-256 fingerprints and support rotation. Snapshots reference their encryption key ID for decryption during restore.

### Local Vault

Mirror snapshots from cloud storage to a local SMB share or USB drive for fast, offline recovery:

```bash
GET /backup/vault/...
```

The vault manager syncs snapshots intelligently (only downloads missing files) and enforces configurable retention (default: 3 most recent snapshots).

### Cloud-to-Cloud Backup (C2C)

Back up Microsoft 365 and Google Workspace data — mailboxes, OneDrive files, SharePoint sites, and Google Drive — with delta sync for incremental updates.
...
| Mode | When | How |
|------|------|-----|
| **Multi-tenant app** (recommended) | `C2C_M365_CLIENT_ID` and `C2C_M365_CLIENT_SECRET` are set in `.env` | One-click "Grant Access" button redirects to Microsoft's admin consent flow |
| **Manual credentials** | Env vars not set | Wizard prompts for Client ID, Client Secret, and Tenant ID from a manually registered Azure AD app |

```bash
# Check if platform app is available
GET /c2c/m365/config
# → { "platformAppAvailable": true }

# Generate consent URL (authenticated)
POST /c2c/m365/consent-url
# → { "url": "https://login.microsoftonline.com/..." }

# Callback (public, CSRF-protected via state)
GET /c2c/m365/callback?code=...&state=...
```

**Connecting Google Workspace:**

Requires a service account key with domain-wide delegation. Configure via the C2C connection wizard.

**Managing connections:**

```bash
GET  /c2c/connections           # List all C2C connections
POST /c2c/connections           # Create connection (manual mode)
GET  /c2c/connections/:id       # Get connection details
POST /c2c/connections/:id/sync  # Trigger delta sync
```

### Disaster Recovery (DR)

DR plans define ordered recovery groups for coordinated failover and rehearsal.

```bash
POST /backup/dr/plans
```

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `name` | string | Yes | Plan name |
| `recoveryGroups` | array | Yes | Ordered groups of devices/services to recover |
| `rpoTarget` | string | No | Target RPO (e.g., `4h`, `1d`) |
| `rtoTarget` | string | No | Target RTO |

Each recovery group specifies devices or services and their recovery order. Groups execute sequentially — the next group starts only after the previous group completes.

**Execution modes:**

| Mode | Description |
|------|-------------|
| `rehearsal` | Dry run — validates recovery steps without modifying production systems |
| `failover` | Live execution — restores systems according to the plan |

```bash
POST /backup/dr/plans/:id/execute
# { "mode": "rehearsal" }

GET /backup/dr/plans/:id/executions
# List past executions with status and timing
```

**SLA Monitoring:**

The DR system monitors RPO and RTO compliance across all protected devices. When a device falls behind its recovery targets, the system generates breach alerts and can trigger automatic escalation.

```bash
GET /backup/dr/sla/status
# Returns RPO/RTO compliance per device and breach counts
```

---

## Enterprise Backup API Reference

| Method | Path | Description |
|--------|------|-------------|
| GET | `/backup/vss/status/:deviceId` | VSS writer status |
| GET | `/backup/mssql/instances` | List discovered SQL Server instances (org-wide) |
| GET | `/backup/mssql/instances/:deviceId` | List SQL instances on a device |
| POST | `/backup/mssql/backup` | Back up a SQL Server database |
| POST | `/backup/mssql/restore` | Restore a SQL Server database |
| GET | `/backup/hyperv/vms` | List Hyper-V VMs (org-wide) |
| GET | `/backup/hyperv/vms/:deviceId` | List VMs on a host |
| POST | `/backup/hyperv/backup` | Export a Hyper-V VM |
...
| POST | `/backup/bmr/token` | Generate BMR recovery token |
| GET | `/backup/bmr/token/:id` | Get recovery token metadata |
| POST | `/backup/verify` | Run backup verification |
| GET | `/backup/verifications` | List verification results |
| GET | `/backup/health` | Organization backup health summary |
| GET | `/backup/recovery-readiness` | Per-device recovery readiness scores |
| GET | `/backup/encryption/keys` | List encryption keys |
| POST | `/backup/encryption/keys` | Create encryption key |
| GET | `/backup/encryption/keys/:id` | Get encryption key metadata |
