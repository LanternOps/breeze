...
<Aside type="note">
  **Third-party (winget) patches and auto-approval.** Third-party application updates arrive without a vendor-assigned severity, so a ring only auto-approves them when its **Patch sources** explicitly include `third_party` (the default is OS-only). Once opted in, ring auto-approval applies to third-party patches just as it does to OS patches. On **Patch Management → Patches**, selecting a specific ring now shows the correct per-source patch counts and operating system for each patch.
</Aside>
