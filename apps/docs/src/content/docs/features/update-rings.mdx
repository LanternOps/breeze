...
<Aside type="tip" title="Default ring is created automatically">
  The first time anyone lists update rings for an organization, Breeze automatically creates a **Default** ring at order 0 with no deferral period and manual approval required for all patches. You do not need to create it yourself. If the Default ring does not appear, verify that the user has organization-scoped access and that the correct organization is selected.
</Aside>
...
<Aside>
  A single device can be covered by multiple configuration policies. If a device matches assignments for more than one ring, the ring with the lowest `ringOrder` takes priority.
</Aside>
