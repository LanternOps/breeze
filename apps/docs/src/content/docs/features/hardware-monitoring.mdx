...
<Aside type="note" title="Requires agent 0.117.0">
  Hardware collection ships in Breeze v0.117.0 and needs the matching agent. A device on an older agent shows no hardware data until its agent is updated.
</Aside>
