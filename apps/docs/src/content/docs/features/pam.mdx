...
In a rule's **Executable** criteria, pick the group from the **Signer group** dropdown instead of typing a single signer — the rule then matches when the executable is signed by *any* publisher in the group. A rule references either a single signer *or* a signer group, not both. A group that's still referenced by a rule can't be deleted; remove those references first.
