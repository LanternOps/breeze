...
<Aside type="note">
  On Windows, the elevation consent dialog shown on the device is drawn on the **secure desktop** — the same isolated desktop Windows itself uses for standard UAC prompts. Because it lives there, other software running in the user's normal session cannot read it, capture it, inject clicks into it, or otherwise spoof or automate the approval.
</Aside>
