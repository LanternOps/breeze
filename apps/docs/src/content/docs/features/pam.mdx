...
<Aside type="caution">
  **Upgrade note: existing auto-approve rules require re-approval.** If your organization had a rule with verdict **Auto-approve** at the time of an upgrade that introduced the dedicated PAM permissions above, that rule is automatically switched to **Require approval** and marked "**Auto-approve suspended -- re-approve**" on the Rules tab -- it keeps matching the same requests (so nothing bypasses your rule chain unnoticed), but every match now waits for a human decision instead of auto-granting. An admin holding `pam:manage_policy` restores the original auto-approve behavior with one click on **Re-approve**. This is a one-time, one-directional migration: it never re-suspends a rule you've already re-approved, and it does not affect the separate Software Policy allowlist/blocklist auto-approval path (evaluated before PAM rules, described above).
</Aside>
