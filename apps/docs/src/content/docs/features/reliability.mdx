...
|--------|--------|-------------|
| Uptime | 30% (servers/infrastructure) / 0% (workstations) | Based on the uptime percentage since the device was enrolled, up to a 90-day window. Score 100 at 100% uptime, linearly down to 0 at 90% or below. Not counted for laptops and desktop workstations (see [Device-type-aware weighting](#device-type-aware-weighting)) |
...
- **Uptime Score**: If uptime is &ge; 100%, score is 100. If &le; 90%, score is 0. Linear interpolation between 90% and 100%. The measurement window starts at the device's enrollment time, so a recently enrolled device is measured only over the time it has actually existed in Breeze — newly enrolled devices no longer show artificially low uptime.
