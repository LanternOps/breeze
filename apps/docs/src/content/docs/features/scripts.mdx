...
| **Secret** | Store the value write-once and masked. See [Secret variables](#secret-variables) — secrets cannot yet be referenced from script content. |
...
Tick **Secret** and the value becomes write-once: it is never displayed again after saving — the list shows `••••••••` and the API returns no value at all. Secret values must be at least four characters long.

<Aside type="caution">
  **Secret variables can be stored, but not yet consumed by a script.** Saving a script whose content references a secret with `{{var.key}}` is rejected, and a run whose parameter is bound to a secret fails on that device rather than substituting a blank. Secure delivery of secret values to the agent is still in development — for now, mark a variable **Secret** to hold the value safely, not to use it from a script. Use a non-secret variable for anything a script has to read today.
</Aside>
