...
description: AI-powered risk scoring, behavior analytics, access reviews, and automated security training for users.
sidebar:
  order: 47
  label: User Risk Scoring
---
...
Each risk score includes a `factors` object that breaks down the score by contributing category. The keys are factor names and the values are the weighted contribution of each factor to the total score.
...
|--------|------|-------------|
| `id` | UUID | Primary key |
| `orgId` | UUID | Organization reference |
| `userId` | UUID | User reference |
| `score` | Integer | Composite risk score (0-100) |
| `factors` | JSONB | Breakdown of contributing factors and their values |
| `trendDirection` | String | Score trend: `up`, `down`, or `stable` |
| `calculatedAt` | Timestamp | When this score was computed |
...
| `orgId` | UUID | Organization reference |
| `userId` | UUID | User reference |
| `eventType` | String | Event category identifier |
| `severity` | String | `low`, `medium`, `high`, or `critical` |
| `scoreImpact` | Integer | Points added to the user's score |
...
|--------|------|-------------|
| `id` | UUID | Primary key |
| `orgId` | UUID | Organization reference (unique) |
| `weights` | JSONB | Factor weight multipliers |
| `thresholds` | JSONB | Score thresholds for risk levels |
| `interventions` | JSONB | Automated intervention settings |
| `updatedBy` | UUID | User who last updated the policy |
