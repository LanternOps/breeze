...
Successful object results from `tools/call` include `structuredContent` matching the redacted JSON in the `text` block. Arrays, scalars, plain text, and error results do not include `structuredContent`.
