...
`initialize` reports the negotiated `protocolVersion`, `serverInfo.version` (the Breeze API release), and `instructions`. Every tool in `tools/list` carries a `title`, MCP `annotations` (`readOnlyHint`, `destructiveHint`, `idempotentHint`, `openWorldHint`) derived from Breeze's guardrail classifications, and `_meta["app.breeze/domain"]`. Annotations are hints for clients; they do not bypass permissions or approval checks.

Successful object results from `tools/call` include `structuredContent` matching the redacted JSON in the `text` block. Pure error objects, non-object results (arrays, scalars, and plain text), and summarized results omit `structuredContent`. The `_chat.outputCompacted` marker identifies a shortened list; natively shaped results retain `structuredContent`.

Batch requests are not supported for any negotiated protocol version. Send one JSON-RPC request per POST. `DELETE /api/v1/mcp/sse` currently returns 204 as a no-op; it does not end a session.

`tools/list` returns a single page by default. Pagination is experimental and enabled only when the operator sets `MCP_TOOLS_LIST_PAGE_SIZE` to a positive value. Every connected client must follow `nextCursor` until it is absent. Cursors are offsets, so a catalog change mid-enumeration can skip tools; tenant-tool discovery failures can also change the list between pages.
...
This URL is the Streamable HTTP endpoint; the client negotiates the protocol version automatically.
...
List tools return `hasMore` and `nextCursor` in their result; when `hasMore` is `true`, call the same tool again with `cursor` set to the previous `nextCursor` and the same filters to get the next page. A result whose payload was too large to return in full is stored as an artifact and comes back with an `artifact.handle` instead -- `read_artifact` pages through that stored copy by byte offset. `read_artifact` only reads an artifact captured by your own chat session or agent run, and capture itself only happens on hosted deployments with the AI workspace enabled or, since v0.117.0, on a self-hosted deployment that opts in with `BREEZE_AI_ARTIFACT_CAPTURE_ENABLED` (see [Environment → AI](/deploy/environment/#ai)); otherwise an oversized result is truncated; an MCP `tools/call` has no session to anchor a capture to, so `read_artifact` over MCP usually returns `no_capture_anchor` today. It is listed for forward compatibility while MCP-side capture anchoring is designed.
...
| `list_incidents` | 1 | List incidents by organization, status, severity, classification, assignee, or date range. |
| `list_remediation_suggestions` | 1 | List remediation suggestions by organization, source, device, or status, limited to accessible devices. |
...
Since v0.117.0 an organization-scoped API key can also see and run its partner's **partner-wide** scripts (those created for "All organizations") through `list_scripts`, `get_script_details` and `run_script`. This is read access only; the key gains no new write access, and it never sees another partner's scripts.

To make sure `run_script` executes exactly the content you reviewed, take `contentSha256` from `get_script_details` and pass it to `run_script` as `expectedContentSha256`. If the script was edited in between, `run_script` refuses with `script_content_mismatch` before anything is dispatched. Omit the field to keep the previous behavior.
...
| `system_cleanup` | 1/3 | List the OS-native maintenance cleaners a device offers — Windows Disk Cleanup handlers and DISM component cleanup, macOS local snapshots and Homebrew, Linux package caches and journal — with an "up to" reclaim estimate (Tier 1). Run the selected actions (Tier 3); `run` returns a `cleanupRunId` immediately and `status` (Tier 1) reads the run's progress and result. |

<Aside type="caution">
  **By default, Tier 3 actions cannot be executed over MCP.** This transport has no interactive approval surface, so `tools/call` answers a Tier 3 action with a structured `MCP_APPROVAL_REQUIRED` error *before* it checks scopes, RBAC, rate limits or the production allowlist — holding `ai:execute` does not change the outcome. `disk_cleanup` and `system_cleanup` stay listed because their read-only actions still work; `tools/list` appends a note to each one's description:

  `(Actions "run" require interactive approval and are not available over MCP — use the Breeze web app AI assistant for those.)`

  A tool whose *every* action is Tier 3 is not listed at all, because advertising a tool that can only ever answer with a refusal is worse than omitting it. The practical contract for an MCP client is therefore: `analyze_disk_usage`, `disk_cleanup` preview and `system_cleanup` list/status give you a complete read-only diagnosis, and you hand the destructive step to a technician in the Breeze web app AI assistant, where it goes through the approval workflow.

  **One operator-controlled exception:** principals named in `MCP_UNATTENDED_TIER3_PRINCIPALS` (`api_key:<id>` or `oauth_client_user:<client_id>/<user id>`) are exempt from this deny for core-registry Tier 3 tools, so the tools are listed and callable for them without the approval step. Everything else still applies to them: `ai:execute`, `ai:execute_admin` in production, the production allowlist, RBAC, rate limits, the Tier 3 execution ledger and the audit log. Tier 4, tenant (BYO MCP) and onboarding bootstrap tools stay approval-only for everyone. Every call that skipped approval this way is marked in its audit row (`approvalBypass: "unattended_principal"` plus the matched `approvalBypassPrincipal`), and the API logs the configured principals at startup. See [Environment variables](/deploy/environment/).
</Aside>
...
| `list_org_contacts` | 1 | List an organization's contacts by site or role, respecting organization and site access. |
| `list_sites` | 1 | List accessible sites with device counts, optionally filtered by organization or name. |
| `get_site` | 1 | Retrieve one accessible site's details by site ID. |
| `list_ai_agents` | 1 | List accessible AI agents and their enabled state, optionally including disabled agents. |
| `list_ai_agent_runs` | 1 | List recent accessible AI agent runs with status, verdict, and cost, filtered by agent or organization. |
| `get_ai_agent_run` | 1 | Retrieve one accessible AI agent run's trace by run ID. |
...
| `list_network_assets` | 1 | List discovered network assets by organization, site, approval status, asset type, or linked device. |
| `get_network_asset` | 1 | Retrieve one accessible discovered network asset's identity, addresses, linked device, and last-seen details. |
...
| `read_artifact` | 1 | Page through a stored large tool-result artifact by byte offset, using the `artifact.handle` from an earlier result. Scoped to your own session or agent run; see the note above "Available tools". |
...
| `manage_ticket_checklist` | 1/2 | Ticket checklist steps and checklist templates. Listing steps and listing or reading templates are Tier 1. Adding, editing (label and detail), deleting and reordering steps, and applying a template, are Tier 2. Requires partner or system scope. Ticking a step done is not available to tools, and a step that is already ticked cannot be edited or deleted through the tool. |
| `list_time_entries` | 1 | List logged work by ticket, user, organization, dates, billing status, or approval. Requires partner or system scope; non-admins see their own entries. |
| `get_running_timer` | 1 | Retrieve the caller's running time entry, or null when no timer is running. Requires partner or system scope and a user principal. |
| `get_timesheet` | 1 | Retrieve a weekly timesheet. Requires partner or system scope and a user principal; another user's timesheet requires admin access. |
...
3. **Tier 3+** tools are included only when *some* action of them is below Tier 3 and the key has `ai:execute`; a wholly Tier 3 tool is never listed — except for a principal named in `MCP_UNATTENDED_TIER3_PRINCIPALS` that also passes the `ai:execute` / `ai:execute_admin` / production-allowlist gates for that tool.
4. A Tier 3 **action** is refused over MCP regardless of scope — see the caution above — unless the caller is a principal named in `MCP_UNATTENDED_TIER3_PRINCIPALS`. For every other principal, `MCP_REQUIRE_EXECUTE_ADMIN` and `MCP_EXECUTE_TOOL_ALLOWLIST` only affect which *tools* a key may see and reach, never whether a Tier 3 action executes; for a named principal they gate its Tier 3 calls as well.
...
These are the defaults. An organisation can raise all of them together with the **Per-tool rate limit multiplier** AI budget setting (a whole number from 1 to 10). Each limit becomes `ceil(limit × multiplier)`, and the multiplier can never lower or remove a limit. Over MCP, the API key's organisation selects the multiplier. A partner-scoped caller with no organisation uses the partner's value. The counter stays per user and per tool. See [AI → Raising the per-tool limits](/features/ai/#raising-the-per-tool-limits).
...
| `MCP_TOOLS_LIST_PAGE_SIZE` | `0` (unset) | Experimental pagination: 0/unset returns a single page. Set to e.g. `50` only when every connected client follows `nextCursor`. Cursors are offsets; a catalog change mid-enumeration can skip tools. |
