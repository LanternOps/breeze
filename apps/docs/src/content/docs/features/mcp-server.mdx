...
List tools return `hasMore` and `nextCursor` in their result; when `hasMore` is `true`, call the same tool again with `cursor` set to the previous `nextCursor` and the same filters to get the next page. A result whose payload was too large to return in full is stored as an artifact and comes back with an `artifact.handle` instead -- `read_artifact` pages through that stored copy by byte offset. `read_artifact` only reads an artifact captured by your own chat session or agent run, and capture itself only happens on hosted deployments with the AI workspace enabled; an MCP `tools/call` has no session to anchor a capture to, so `read_artifact` over MCP usually returns `no_capture_anchor` today. It is listed for forward compatibility while MCP-side capture anchoring is designed.
