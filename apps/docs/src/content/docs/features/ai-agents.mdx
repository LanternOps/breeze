...
A **Fleet designer** agent (kind `designer`, v0.113.0+) reads the whole fleet and writes a justified monitoring design for a technician to approve — nothing more. It reaches every read-only tool plus one report tool, has no mutating capabilities to choose, and has no shadow mode because there is nothing to shadow — the mode picker offers only **Off** and **On** for this kind, and a new designer starts on **On** ("run and write reports").
...
You do not have to create the agent by hand. If the organization's partner has no Fleet designer yet — or has one that is off — the **Fleet Design** page says so and offers **Enable Fleet Designer**, which creates the partner-wide designer agent on **On** with you as its recipient (or turns the existing one on). The button appears only for a partner administrator with `ai_agents:write`; an organization-scoped user sees who to ask instead.

<Aside>
  A Fleet designer resolves through the partner-wide agent, so an organization can tighten it (turn it off for one customer) but cannot create it alone. Nothing in a design is applied to the fleet automatically until a technician approves it.
</Aside>
