...
| **Executes unattended** | Act mode only, and only for the fixed, act-eligible operations described in [Act mode](#act-mode). **Run a script** is the one exception: it stays an approval request until at least one script is authorized for the agent (`actAssets.scriptIds`, set through the API today — there is no form control for it yet), and the picker and review card say so rather than promising an unattended run. |
