...
<Aside>
  The internal database status `completed` is returned as `success` in all API responses.
</Aside>
...
Each run includes a `logs` array with per-action, per-device log entries:
...
| `actionType` | The action type that produced this entry |
| `actionIndex` | Zero-based index of the action in the automation's action list |
| `deviceId` | The device this entry relates to (if applicable) |
| `commandId` | The command ID dispatched to the agent (for `run_script` and `execute_command`) |
| `alertId` | The created alert ID (for `create_alert`) |
| `channelId` | The notification channel ID (for `send_notification`) |
...
**Schedule not firing.**
Verify the cron expression is valid and the timezone is a recognized IANA timezone name. Confirm the automation is enabled — disabled automations do not fire on schedule. Use `POST /:id/trigger` to confirm the actions work correctly independently of the schedule mechanism.

**Event automation not firing.**
The `eventType` field is case-sensitive — `alert.created` and `Alert.Created` are different values. Confirm the automation is enabled and that its policy assignment covers the device generating the event. Use `POST /:id/trigger` to verify the action configuration is valid separately from the event matching.
