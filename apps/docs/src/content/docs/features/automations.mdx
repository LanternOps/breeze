...
| `whenOffline` | No | What to do when a target device is offline: `queue` (default) delivers the run automatically the next time the device checks in, up to a 7-day deadline; `skip` fails that device immediately instead of waiting. |
...
| `whenOffline` | No | What to do when a target device is offline: `queue` (default) delivers the command automatically the next time the device checks in, up to a 7-day deadline; `skip` fails that device immediately instead of waiting. |
