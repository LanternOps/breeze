...
3. Enter the endpoint URL. It must be `https`. Breeze sends your credential **only** to that URL's origin; a redirect elsewhere is followed without it.
...
- **Review needed** — the upstream definition changed since you last looked. The tool keeps working at the tier you set; the flag clears when you re-tier or re-enable it.
...
- **401 / 403 from the server** — the stored credential was rejected. Edit the source and re-enter it; leaving a credential field blank keeps the stored one.
