...
Each detected tool is reported with one of two statuses:
...
<Aside>
  A tool that is neither installed nor running produces no entry — it is simply absent from the results. Undetected tools do not appear as `unknown`; they are omitted entirely.
</Aside>
...
| **macOS** | Process checks, launchd/launchctl status, `profiles status -type enrollment` (MDM enrollment state), `dsconfigad` (Active Directory binding) |
| **Linux** | Basic process checks (expanding in future releases) |
...
**Linux shows fewer results than Windows or macOS**

Linux detection is currently limited to process checks. The service query, registry, and launchd-based detection methods used on Windows and macOS are not applicable on Linux. Broader Linux detection is planned for a future release.
