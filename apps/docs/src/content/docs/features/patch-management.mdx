...
| **`rejected`** | Explicitly declined — will not be installed. Non-destructive; can be reversed by approving the patch at any time. |
...
Rejection creates an audit record but is not a permanent block. A rejected patch can be approved at any time by calling the approve endpoint. No data is deleted when a patch is rejected.
...
If `scheduleType` is `"scheduled"`, also provide a `scheduledTime` in ISO 8601 format:
...
Available query parameters: `orgId`, `source`, `severity`, `format` (`csv` or `pdf`).
...
This returns a file stream in the format requested (CSV or PDF). Attempting to download before the report is `completed` will return an error.
