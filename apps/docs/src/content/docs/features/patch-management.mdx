...
- **`pendingCommandIds`** — subset queued but not yet dispatched (device offline); delivered automatically on the device's next heartbeat, up to a 7-day deadline, and cancellable from the device's **Queued actions** list
