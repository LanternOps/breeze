...
| `rebootDeferralMinutes` | integer | How far each postponement pushes the restart out (5–1440 minutes). Default: `60`. `rebootMaxDeferrals × rebootDeferralMinutes` may not exceed `10080` (7 days) — the agent refuses a restart scheduled further out than that |
