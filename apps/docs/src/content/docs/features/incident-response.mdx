...
The `execute_containment` tool queues an agent command. If the target device is offline, the command remains pending until the device reconnects. Check device connectivity.
