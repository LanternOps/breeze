---
title: Cloud Tenant Integrations — Google Workspace & Microsoft 365
description: Connect Google Workspace and Microsoft 365 (Entra) tenants on the Integrations → Cloud tenants tab so Breeze and its AI assistant can administer users and groups.
---

import { Steps, Aside, Tabs, TabItem } from '@astrojs/starlight/components';

The **Cloud tenants** tab of the Breeze Integrations page (formerly **Identity**) is where you connect a customer's Google Workspace or Microsoft 365 tenant to Breeze. It has two sub-tabs — **Google Workspace** and **Microsoft 365**. Microsoft 365 offers two separate connection methods: **Customer Graph Read** uses customer-administrator consent without asking you for a reusable secret, while **Legacy direct** accepts a tenant-specific app client secret for existing read and action workflows.

Open it at **Integrations → Cloud tenants** (`/integrations#cloud-tenants`; the old `/integrations#identity` link still works). The two sub-tabs deep-link to `/integrations#google` (Google Workspace) and `/integrations#m365` (Microsoft 365).
...
<Aside type="caution" title="Off by default">
  Both provider sub-tabs are feature-flagged off by default. A host operator must enable `GOOGLE_WORKSPACE_ENABLED` (Google Workspace) and/or `M365_ENABLED` (Microsoft 365). Customer Graph Read has an additional organization allowlist; if it is not enabled for your organization and nothing is connected yet, its card shows only "Not available on this Breeze instance yet." Host operators self-hosting Breeze: the environment variables that turn Customer Graph Read on are documented under [Customer Microsoft 365 Graph-read consent](/deploy/environment/#customer-microsoft-365-graph-read-consent-optional).
</Aside>
...
<Steps>
1. Set your scope to the customer organization, then open **Integrations → Cloud tenants → Google Workspace**.
...
<Steps>
1. Set your scope to the customer organization, then open **Integrations → Cloud tenants → Microsoft 365**.
...
**The Customer Graph Read or Customer Graph Actions card shows "Not available on this Breeze instance yet."**
Onboarding for that profile is turned off on this Breeze instance, or your organization is not on its allowlist. It is not something you can change from the organization. Self-hosting operators enable it with the variables under [Customer Microsoft 365 Graph-read consent](/deploy/environment/#customer-microsoft-365-graph-read-consent-optional); this also requires the Graph executor sidecar to be deployed.
