...
<Aside>
  Block and read-only enforcement currently operates in alert-only mode on all platforms. The agent logs the policy decision and reports it to the server, but kernel-level enforcement (preventing device access or mounting read-only) requires a platform-specific driver that is not yet included. The `details` field on blocked events includes `"enforcement": "alert_only"` to indicate this.
</Aside>
