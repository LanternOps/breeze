...
When an organization has [Microsoft 365 Graph read](/features/identity-integrations/#customer-graph-read-recommended-for-read-only-inventory) connected, the AI assistant can answer questions about that customer's Microsoft 365 tenant directly. All six tools are **read-only** and pull **live** data from the connected tenant — nothing is written back to Microsoft 365. They only appear in a session when Microsoft 365 Graph read is enabled for the organization.

| Tool | Tier | Description |
|------|------|-------------|
| `m365_query_users` | 1 | List or look up Entra ID (Azure AD) users — display name, UPN, mail, account status, department |
| `m365_query_signins` | 1 | Query recent Entra ID sign-in activity, optionally for one user (up to the last 168 hours). Requires the tenant to have Entra ID P1 or P2 |
| `m365_query_intune_devices` | 1 | List or look up Intune-managed devices — OS, compliance state, last sync, owner |
| `m365_query_groups` | 1 | List groups, look up a group, or list a group's members |
| `m365_query_org` | 1 | Read the tenant's organization profile or its assigned/consumed license (SKU) inventory |
| `m365_query_sites` | 1 | Search SharePoint sites or look one up by id |

These tools are exposed in both the built-in assistant and to external AI clients through the [MCP Server](/features/mcp-server/#microsoft-365-tools). See [Identity Integrations](/features/identity-integrations/) for how a customer administrator grants the read-only Graph consent.
