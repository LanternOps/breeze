...
| `DATABASE_URL_APP` | — | One of these three (prod) | Explicit connection string for the **unprivileged request role** (`breeze_app`). Preferred for direct (non-Compose) deployments, and **required** for multi-host / high-availability `DATABASE_URL` values, which cannot be derived automatically. **Leave unset on the bundled `docker-compose.yml`** — see the warning below. |
...
:::caution[Docker Compose: leave `DATABASE_URL_APP` unset]
...
loopback**. Migrations and role setup still succeed (they use `DATABASE_URL`),
then startup fails at the initial seed:
...
Comment the line out (or set it empty) and Breeze derives
`breeze_app@postgres` from the container's own `DATABASE_URL`. Only set it
explicitly when pointing at a database the api container can actually reach.
:::
