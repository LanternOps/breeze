...
<Aside type="caution" title="Keep your compose file current">
  The stock `docker-compose.yml` hand-maps each variable into the `api` service's `environment:` block — a value in `.env` that isn't mapped there **silently does nothing**. Compose files from releases before v0.99.0 were missing the mappings for a number of documented variables (including `ENABLE_2FA`, `BREEZE_PLATFORM_ADMINS`, `CORS_ALLOWED_ORIGINS`, the Twilio SMS, Mailgun, and several M365 families). If a documented setting appears to have no effect, update to the current compose file — or add the variable to the `environment:` block yourself.
</Aside>
...
| `APP_ENCRYPTION_KEY_ID` | — | | Optional identifier for the active encryption key (letters, digits, `.`/`_`/`-`). Setting it enables versioned, context-bound ciphertext for newly stored secrets and is **required for the Microsoft 365 reset-password one-time credential reveal** — without it the reset still succeeds but the temporary password is deliberately discarded rather than stored. Not present in the stock compose file: add it to `.env` **and** to the `api` service `environment:` block. |
...
Per-user and per-endpoint rate limits (login, MFA, dashboard usage) are fixed in code and not configurable via environment variables. The one deployment-tunable limit is the per-org agent cap, which bounds how much traffic a single tenant's fleet can generate, even with thousands of agents.

| Variable | Default | Description |
|---|---|---|
| `AGENT_ORG_RATE_LIMIT_PER_MIN` | `600` | Per-organization sliding-window rate limit on agent-authenticated endpoints. Returns `429` with `Retry-After: 60` when exceeded. Sized for ~5 active agents per org; raise for MSPs with very large fleets. Not mapped in the stock compose file — to change it, add it to the `api` service `environment:` block as well as `.env`. |
