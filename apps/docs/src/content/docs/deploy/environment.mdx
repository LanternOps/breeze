...
| `BREEZE_AI_AGENTS_ENABLED` | `false` | Surface **AI Agents** (Settings → AI Agents). Off by default — no agent exists, runs, or appears in the UI until this is `true`. Even when enabled, agents only observe and propose in this release; nothing runs unattended. Must also be mapped in the `api` service's `environment:` block, not just set in `.env`. |
