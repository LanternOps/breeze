...
| `DEVICE_COMMAND_OFFLINE_QUEUE_ENABLED` | `true` | Fire-and-forget device actions (scripts, patch installs, software installs, reboots, automations, …) issued against an offline device are queued and delivered on its next successful heartbeat, up to a per-action-type deadline (7 days for most types; 24 hours for inventory/config-sync and power-state actions). Set `false` to go back to the pre-#5128 behavior of rejecting the action outright when the device is offline. |
