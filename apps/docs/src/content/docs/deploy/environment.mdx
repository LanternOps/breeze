...
| Variable | Default | Description |
|---|---|---|
| `TICKETS_INBOUND_DOMAIN` | — | The domain inbound ticket addresses live on, e.g. `tickets.yourmsp.com`. Each partner's address is `<local part>@<this domain>`. **Until this is set, the Inbound email card reports "The platform inbound domain isn't configured yet"** and no address can be generated. |
