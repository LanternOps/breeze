...
| `PARTNER_API_CURSOR_SIGNING_KEY` | — | Yes (prod) | Dedicated key that signs the pagination cursors returned by the [Partner Export API](/reference/api/#partner-export-api). Must be **canonical (standard) base64** — not base64url or hex — decoding to at least 32 bytes, and must differ from `JWT_SECRET`. Generate: `openssl rand -base64 32`. The API refuses to start in production without it. |
