...
### v0.110.0 — AI agent graduation and impact accounting

**No new environment variables.** Both features run under the flags you already have — `BREEZE_AI_AGENTS_ENABLED` for the nightly impact rollup, and `BREEZE_AI_AGENTS_POLICY_DECIDE_ENABLED` (still off, still not generally available) for promoting an operation to pre-authorized execution. Automatic demotion is not gated by anything and is always on.
