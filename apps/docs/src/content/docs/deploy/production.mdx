...
`.env.example` ships with an explicit `BREEZE_VERSION` pin, so a fresh deployment is already pinned. Change it deliberately when you upgrade; if you clear it, images fall back to the `latest` tag. To pin to a specific release:
