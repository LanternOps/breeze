...
| **Coturn** | `coturn/coturn:4-alpine` | TURN relay server for WebRTC remote desktop |
...
3. **Set your domain and secrets**
   ...
   Then set the TURN server's public IP (required for remote desktop to work across NATs):

   ```bash
   # Set to this server's public IP address
