...
<Aside type="note">
  In both modes, registering a release also promotes it to the fleet upgrade target by default. Set `AGENT_AUTO_PROMOTE=false` to register binaries without promoting them, then promote explicitly once you've verified — see [Controlled agent fleet rollout](/deploy/upgrades/#controlled-agent-fleet-rollout).
</Aside>
