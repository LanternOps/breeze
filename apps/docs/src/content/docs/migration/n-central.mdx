...
**Service Templates** are N-central's bundle of service monitors with thresholds, applied by device class or filter. Rebuild them as Breeze [monitors](/features/monitors/) with [Delivery](/features/notifications/), defined partner-wide so one definition covers every customer.
