...
2. Put `EXPO_PUBLIC_SENTRY_DSN`, `EXPO_PUBLIC_API_URL`, and (when approved) `EXPO_PUBLIC_POSTHOG_KEY` / `EXPO_PUBLIC_POSTHOG_HOST` in **`apps/mobile/.env`** — the file, not your shell, because Xcode build phases do not inherit it. See `.env.example` for what each one does when left unset. `EXPO_PUBLIC_SENTRY_DSN` is no longer optional for a release build: the Archive fails without it (see "Sentry — telemetry" above).
3. Run `npx pnpm@10.33.4 --filter=breeze-mobile preflight`. **This is an optional convenience, not a gate** — nothing in the repo invokes it, and Xcode will never run it for you. It is worth running anyway because it catches two things the build-time guard does not: `EXPO_PUBLIC_API_URL` pointing at `localhost` (silently fatal on a real device — every request fails), and a missing `SENTRY_AUTH_TOKEN` (not silent, but it fails ten minutes into an archive instead of instantly here). The DSN check it also performs is now redundant with the `app.config.js` guard, which cannot be skipped.
