...
export const ROUTE_SCOPES: Array<{ pattern: RegExp; kind: RouteScopeKind }> = [
  ...
  // Public Quick Support landing. The one-time code in the URL is the only
  // credential — an end user reaching it has no Breeze account at all.
  // /remote-access is canonical; /quick is a permanent 301 stub (quick.astro).
  { pattern: /^\/(quick|remote-access)$/, kind: 'auth' },
];
