...
describe('ReportTemplates — saved reports merged into curated cards', () => {
  ...
  it('renders a separate card per saved report when several share a curated name', async () => {
    ...
    // Total template count grows by one: 8 curated cards (#5784 W02 added
    // Threat Detection Review, W03 added the Intune endpoint-management
    // card, W04 added the Vulnerability Management card), with the
    // hardware-lifecycle slot expanded from 1 card to 2.
    expect(screen.getAllByRole('button', { name: 'Use template' })).toHaveLength(9);
  });
