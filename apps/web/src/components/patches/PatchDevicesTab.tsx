...
export default function PatchDevicesTab() {
  const [devices, setDevices] = useState<DevicePatchSummary[]>([]);
  ...
  const fetchDevices = useCallback(async () => {
    try {
      ...
      const merged: DevicePatchSummary[] = [];
  ...
  const handleBulkScan = useCallback(async () => {
    ...
    try {
      ...
      setBulkSuccess(`Patch scan started on ${ids.length} ${ids.length === 1 ? 'device' : 'devices'}`);
      clearSelection();
      setTimeout(() => { void fetchDevices(); }, 3000);
    } catch (err) {
      setBulkError(err instanceof Error ? err.message : 'Failed to start scan');
    } finally {
      setBulkAction(null);
    }
  }, [selectedIds, clearSelection, fetchDevices]);
  ...
  }, [selectedIds, devices, clearSelection, fetchDevices]);
