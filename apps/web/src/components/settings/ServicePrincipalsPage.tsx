...
export default function ServicePrincipalsPage() {
  ...
  const load = useCallback(async () => {
    ...
    setLoadError(false);
    try {
      const response = await fetchWithAuth('/service-principals');
    ...
    try {
      await runAction({
        request: () => fetchWithAuth(
          creating ? '/service-principals' : `/service-principals/${editing.id}`,
          { method: creating ? 'POST' : 'PATCH', body: JSON.stringify(payload) },
        ),
        errorFallback: t('servicePrincipals.saveFailed'),
        successMessage: t(creating ? 'servicePrincipals.created' : 'servicePrincipals.updated'),
        onUnauthorized: UNAUTHORIZED,
      });
      setEditing(null);
      await load();
    } catch (error) {
      handleActionError(error, t('servicePrincipals.saveFailed'));
    }
  };
  ...
  const disablePrincipal = async (principal: Principal) => {
    try {
      await runAction({
        request: () => fetchWithAuth(`/service-principals/${principal.id}`, {
          method: 'PATCH', body: JSON.stringify({ status: 'disabled' }),
        }),
        errorFallback: t('servicePrincipals.disableFailed'),
        successMessage: t('servicePrincipals.disabled'),
        onUnauthorized: UNAUTHORIZED,
      });
      await load();
    } catch (error) {
      handleActionError(error, t('servicePrincipals.disableFailed'));
    }
  };
  ...
  const issueKey = async () => {
    ...
    try {
      const issued = await runAction<{ key: string }>({
        request: () => fetchWithAuth(`/service-principals/${issueTarget.id}/keys`, {
          method: 'POST', body: JSON.stringify({ name: keyName.trim() }),
        }),
        errorFallback: t('servicePrincipals.issueFailed'),
        successMessage: t('servicePrincipals.issued'),
        onUnauthorized: UNAUTHORIZED,
      });
      setIssueTarget(null);
      setKeyName('');
      ...
      handleActionError(error, t('servicePrincipals.issueFailed'));
    }
  };
  ...
  const rotateKey = async (principal: Principal, key: PrincipalKey) => {
    try {
      const rotated = await runAction<{ key: string }>({
        request: () => fetchWithAuth(`/service-principals/${principal.id}/keys/${key.id}/rotate`, { method: 'POST' }),
        errorFallback: t('servicePrincipals.rotateFailed'),
        successMessage: t('servicePrincipals.rotated'),
        onUnauthorized: UNAUTHORIZED,
      });
      setNewKey(rotated.key);
      await load();
    } catch (error) {
      handleActionError(error, t('servicePrincipals.rotateFailed'));
    }
  };
  ...
  const revokeKey = async (principal: Principal, key: PrincipalKey) => {
    try {
      await runAction({
        request: () => fetchWithAuth(`/service-principals/${principal.id}/keys/${key.id}`, { method: 'DELETE' }),
        errorFallback: t('servicePrincipals.revokeFailed'),
        successMessage: t('servicePrincipals.revoked'),
        onUnauthorized: UNAUTHORIZED,
      });
      await load();
    } catch (error) {
      handleActionError(error, t('servicePrincipals.revokeFailed'));
    }
  };
  ...
  return <><div className="space-y-6" data-testid="service-principals-page">
    ...
    {principals.length === 0 && <p className="rounded border p-6 text-center text-muted-foreground">{t('servicePrincipals.empty')}</p>}
    {principals.map((principal) => <section key={principal.id} className="rounded-lg border bg-card p-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <div className="flex gap-2">
          <button className="rounded border px-3 py-1.5 text-sm" onClick={() => openEdit(principal)}>{t('servicePrincipals.edit')}</button>
          {principal.status === 'active' && <button data-testid={`disable-principal-${principal.id}`} className="rounded border px-3 py-1.5 text-sm" onClick={() => void disablePrincipal(principal)}>{t('servicePrincipals.disable')}</button>}
          <button data-testid={`issue-key-${principal.id}`} disabled={principal.status !== 'active'} className="rounded border px-3 py-1.5 text-sm" onClick={() => { setIssueTarget(principal); setKeyName(''); }}>{t('servicePrincipals.issueKey')}</button>
        </div>
      </div>
      <div className="mt-4 overflow-x-auto"><table className="w-full text-sm"><thead><tr className="border-b text-left"><th className="py-2">{t('servicePrincipals.keyName')}</th><th>{t('servicePrincipals.prefix')}</th><th>{t('servicePrincipals.status')}</th><th className="text-right">{t('servicePrincipals.actions')}</th></tr></thead><tbody>
        {principal.keys.map((key) => <tr key={key.id} className="border-b last:border-0"><td className="py-2">{key.name}</td><td className="font-mono">{key.keyPrefix}…</td><td>{key.status}</td><td className="space-x-2 text-right">{key.status === 'active' && <><button data-testid={`rotate-key-${key.id}`} className="underline" onClick={() => void rotateKey(principal, key)}>{t('servicePrincipals.rotate')}</button><button data-testid={`revoke-key-${key.id}`} className="underline" onClick={() => void revokeKey(principal, key)}>{t('servicePrincipals.revoke')}</button></>}</td></tr>)}
      </tbody></table></div>
    </section>)}
    ...
    {editing && <div className="fixed inset-0 z-50 grid place-items-center bg-background/80 p-4"><div className="w-full max-w-lg space-y-4 rounded-lg border bg-card p-6 shadow-lg">
      <h2 className="font-semibold">{t(editing === 'new' ? 'servicePrincipals.create' : 'servicePrincipals.edit')}</h2>
      <label className="block text-sm">{t('servicePrincipals.name')}<input className="mt-1 w-full rounded border bg-background p-2" value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} /></label>
      <label className="block text-sm">{t('servicePrincipals.principalDescription')}<textarea className="mt-1 w-full rounded border bg-background p-2" value={draft.description} onChange={(event) => setDraft({ ...draft, description: event.target.value })} /></label>
      <fieldset><legend className="text-sm">{t('servicePrincipals.scopes')}</legend><div className="mt-1 grid grid-cols-2 gap-1">{AVAILABLE_SCOPES.map((scope) => <label key={scope} className="text-xs"><input type="checkbox" checked={draft.scopes.includes(scope)} onChange={(event) => setDraft({ ...draft, scopes: event.target.checked ? [...draft.scopes, scope] : draft.scopes.filter((item) => item !== scope) })} /> {scope}</label>)}</div></fieldset>
      <label className="block text-sm">{t('servicePrincipals.sourceCidrs')}<textarea className="mt-1 w-full rounded border bg-background p-2 font-mono" value={draft.sourceCidrs} onChange={(event) => setDraft({ ...draft, sourceCidrs: event.target.value })} /></label>
      <label className="block text-sm">{t('servicePrincipals.expiresAt')}<input type="datetime-local" className="mt-1 w-full rounded border bg-background p-2" value={draft.expiresAt} onChange={(event) => setDraft({ ...draft, expiresAt: event.target.value })} /></label>
      <div className="flex justify-end gap-2"><button className="rounded border px-3 py-2" onClick={() => setEditing(null)}>{t('servicePrincipals.cancel')}</button><button className="rounded bg-primary px-3 py-2 text-primary-foreground" onClick={() => void savePrincipal()}>{t('servicePrincipals.save')}</button></div>
    </div></div>}
