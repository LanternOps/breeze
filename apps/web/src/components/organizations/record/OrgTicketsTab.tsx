...
export default function OrgTicketsTab({ orgId, orgFetch }: OrgTicketsTabProps) {
  ...
  const ticketsLatest = useLatest<TicketSummary[] | 'failed'>();
  ...
  const load = useCallback(async () => {
    ...
    const result = await ticketsLatest
      .run(
        orgFetch(`/tickets?${params.toString()}`).then(async (res) => {
          if (!res.ok) return 'failed' as const;
          const body = (await res.json()) as { data?: TicketSummary[] };
  ...
  return (
    <div data-testid="org-tickets-tab" className="space-y-3">
      <div className="flex items-center justify-between gap-3">
        <div className="flex items-center gap-1 border-b" data-testid="org-tickets-status-tabs">
          {STATUS_TABS.map((id) => (
            <button
              key={id}
              type="button"
