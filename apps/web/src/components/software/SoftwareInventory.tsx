...
export default function SoftwareInventory({
  ...
  const [sortBy, setSortBy] = useState<
    "deviceCount" | "name" | "vendor" | "lastSeen"
  >("deviceCount");
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error(
          i18n.t(
            "policies:software.softwareInventory.failedToLoadSoftwareInventory",
          ),
        );
  ...
  const handleApprove = async (name: string, vendor: string | null) => {
    try {
      ...
      if (!res.ok)
        throw new Error(
          i18n.t("policies:software.softwareInventory.failedToApprove"),
        );
  ...
  const handleDeny = async (name: string, vendor: string | null) => {
    try {
      ...
      if (!res.ok)
        throw new Error(
          i18n.t("policies:software.softwareInventory.failedToDeny"),
        );
      ...
      showToast({
        type: "error",
        message: err instanceof Error ? err.message : "Failed to deny",
      });
    }
  };
  const handleClear = async (name: string, vendor: string | null) => {
    try {
      ...
      if (!res.ok)
        throw new Error(
          i18n.t("policies:software.softwareInventory.failedToClearStatus"),
        );
      showToast({
        type: "success",
        message: `"${name}" policy status cleared`,
      });
      await fetchData();
    } catch (err) {
      showToast({
        type: "error",
        message: err instanceof Error ? err.message : "Failed to clear status",
      });
    }
  };
      ...
      <div className="flex items-center gap-4">
        <div className="relative flex-1 max-w-md">
          ...
          <input
            type="text"
            placeholder={i18n.t(
              "policies:software.softwareInventory.searchSoftware",
            )}
            value={search}
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-muted/30 text-left text-xs uppercase tracking-wide text-muted-foreground">
              <tr>
                ...
                <th className="px-4 py-3">
                  {i18n.t("policies:software.softwareInventory.versions")}
                </th>
                <th className="px-4 py-3">
                  {i18n.t("policies:software.softwareInventory.policyStatus")}
                </th>
            ...
            <tbody>
              {loading && data.length === 0 ? (
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center">
                    ...
                    <p className="mt-2 text-sm text-muted-foreground">
                      {i18n.t(
                        "policies:software.softwareInventory.loadingSoftwareInventory",
                      )}
                    </p>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={6} className="px-4 py-12 text-center">
                    ...
                    <p className="mt-2 text-sm font-medium text-muted-foreground">
                      {i18n.t(
                        "policies:software.softwareInventory.noSoftwareDataCollectedYet",
                      )}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {i18n.t(
                        "policies:software.softwareInventory.softwareInventoryIsCollectedAutomaticallyFromEnrolled",
                      )}
                    </p>
                  </td>
                </tr>
                ...
                data.map((row) => {
                  const badge =
                    policyBadge[row.policyStatus] ?? policyBadge.no_policy;
                  return (
                    <tr
                        ...
                        <div className="flex flex-wrap gap-1">
                          ...
                          {row.versions.length > 3 && (
                            <span className="inline-flex items-center text-xs text-muted-foreground">
                              +{row.versions.length - 3}
                              {i18n.t(
                                "policies:software.softwareInventory.more",
                              )}
                            </span>
                          )}
                          ...
                          {actionMenu?.name === row.name && (
                            ...
                            >
                              ...
                              >
                                ...
                                {i18n.t(
                                  "policies:software.softwareInventory.approve",
                                )}
                              </button>
                              <button
                                type="button"
                              ...
                              {(row.policyStatus === "allowed" ||
                                ...
                                >
                                  ...
                                  {i18n.t(
                                    "policies:software.softwareInventory.clearStatus",
                                  )}
                                </button>
                              )}
                              <button
                              ...
                              >
                                ...
                                {i18n.t(
                                  "policies:software.softwareInventory.createPolicy",
                                )}
                              </button>
                            </div>
                          )}
