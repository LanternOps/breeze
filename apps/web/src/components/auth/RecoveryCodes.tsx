  ...
  return (
    <div className="space-y-6 rounded-lg border bg-card p-6 shadow-xs">
      <div className="space-y-2">
        <h2 className="text-lg font-semibold">{t('recoveryCodes.title', { defaultValue: 'Save your recovery codes' })}</h2>
        ...
        >
          {copied ? (
            <>
              ...
              {t('recoveryCodes.copied', { defaultValue: 'Copied!' })}
            </>
          ) : (
            <>
      ...
      <div className="rounded-md border border-yellow-500/40 bg-yellow-500/10 px-3 py-2 text-sm text-yellow-600 dark:text-yellow-400">
        <strong>{t('recoveryCodes.importantLabel', { defaultValue: 'Important:' })}</strong>{' '}
        {t('recoveryCodes.importantDescription', {
          defaultValue: 'Store these codes in a secure location. You will not be able to see them again.',
        })}
      </div>
      ...
      {onContinue && (
        ...
        >
          {t('recoveryCodes.savedCodes', { defaultValue: 'I have saved my codes' })}
        </button>
      )}
    </div>
  );
