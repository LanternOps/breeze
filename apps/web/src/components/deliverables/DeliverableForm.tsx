...
export default function DeliverableForm({
  ...
  // tuple was empty in W01 and each later wave appends one type; once it is
  ...
  // literal tuple, so once it has members TypeScript narrows `.length` to `1`
  // and `=== 0` becomes a ts(2367) "no overlap" error. The guard itself must
  // stay — the tuple can legitimately be empty again if a type is retired.
  useEffect(() => {
