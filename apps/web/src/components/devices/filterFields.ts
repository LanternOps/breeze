...
export const V2_FILTER_FIELDS: FilterFieldDefinition[] = [
  ...
  { key: 'lastUser', label: 'Last User', category: 'core', type: 'string', operators: OP_STRING },
  { key: 'isHeadless', label: 'Headless', category: 'core', type: 'boolean', operators: ['equals', 'notEquals'] },
  { key: 'uptimeSeconds', label: 'Uptime (seconds)', category: 'core', type: 'number', operators: OP_NUMBER },
  { key: 'watchdogStatus', label: 'Watchdog Status', category: 'core', type: 'enum', operators: OP_ENUM, enumValues: ['connected', 'failover', 'offline'] },
  { key: 'quarantinedAt', label: 'Quarantined At', category: 'core', type: 'datetime', operators: OP_DATETIME },
  { key: 'lastSeenIp', label: 'Last Seen IP', category: 'network', type: 'string', operators: OP_STRING },
  ...
  // Device-state predicates — virtual EXISTS fields (server resolves these
  // against device_patches / alerts / patch_job_results). Single 'yes' value
  // drives the quick-add chips; server also accepts notEquals for negation.
