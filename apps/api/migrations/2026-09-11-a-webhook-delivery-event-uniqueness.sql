...
-- uniqueness on it, and the '*' event subscriber in workers/webhookDelivery.ts
-- creates a delivery row and queues an outbound POST unconditionally. A
-- redelivered event therefore produces a SECOND POST to the CUSTOMER's
-- endpoint — the only externally-visible duplicate in the wave-3.5 idempotency
-- audit, and the one that cannot be walked back once it has been sent.
--
...
-- Keep the FIRST row per pair (it carries the original attempt/response
-- history) and report the count: silently discarding delivery history destroys
-- the forensic trail, and a non-zero count here is itself evidence that
-- duplicate delivery already happens in production.
DO $$
DECLARE
  removed integer;
BEGIN
