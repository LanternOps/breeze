...
describe('escalation fan-out bounds', () => {
  it.each([{ everyMinutes: 1441, maxTimes: 1 }, { everyMinutes: 1, maxTimes: 11 },
    ...
    { everyMinutes: 1.5, maxTimes: 1 }, { everyMinutes: 1, maxTimes: 1.5 }])('rejects repeat %j', repeat => {
    expect(escalationStepSchema.safeParse({ ...step, repeat }).success).toBe(false);
  });
  it.each([{ everyMinutes: 1, maxTimes: 1 }, { everyMinutes: 1440, maxTimes: 10 }])('accepts repeat boundary %j', repeat => {
    expect(escalationStepSchema.safeParse({ ...step, repeat }).success).toBe(true);
  });
  it.each([createPolicySchema, updatePolicySchema])('limits total policy occurrences on create and update', schema => {
    const steps = Array.from({ length: 5 }, () => ({ ...step, repeat: { everyMinutes: 1, maxTimes: 9 } }));
