...
describe('org routes', () => {
  ...
  describe('PATCH /orgs/partners/me', () => {
    ...
    // Quick Support custom domain: stored as partners.settings.quickSupportDomain
    // and interpolated into `https://<value>/quick?code=…` when a support session
    // is minted, so the write path must accept ONLY a bare hostname.
    describe('settings.quickSupportDomain', () => {
      function mockPartnerRowAndCaptureUpdate(currentSettings: Record<string, unknown> = {}) {
      ...
      function patchDomain(quickSupportDomain: unknown) {
        return app.request('/orgs/partners/me', {
          method: 'PATCH',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ settings: { quickSupportDomain } }),
        });
      }

      it('accepts a bare hostname and persists it', async () => {
        setAuthContext({ scope: 'partner', partnerId: 'partner-123' });
        const captured = mockPartnerRowAndCaptureUpdate();

        const res = await patchDomain('support.acme.com');

        expect(res.status).toBe(200);
        expect(captured.data.settings.quickSupportDomain).toBe('support.acme.com');
      });

      it('trims and lowercases on write', async () => {
        setAuthContext({ scope: 'partner', partnerId: 'partner-123' });
        const captured = mockPartnerRowAndCaptureUpdate();
      ...
      it.each([
        ...
        ['   ', 'whitespace only'],
        [null, 'explicit null'],
      ])('clears the setting for %j (%s)', async (value: string | null, _label: string) => {
        setAuthContext({ scope: 'partner', partnerId: 'partner-123' });
        const captured = mockPartnerRowAndCaptureUpdate({ quickSupportDomain: 'support.acme.com' });

        const res = await patchDomain(value);

        expect(res.status).toBe(200);
        expect(captured.data.settings.quickSupportDomain).toBeNull();
      });

      it.each([
        'https://support.acme.com',
        'support.acme.com/path',
        'support.acme.com:8443',
        'support acme.com',
        'user@support.acme.com',
        'localhost',
        '-bad.acme.com',
        `${'a'.repeat(250)}.acme.com`,
      ])('rejects %j with 400', async (bad) => {
        setAuthContext({ scope: 'partner', partnerId: 'partner-123' });

        const res = await patchDomain(bad);
