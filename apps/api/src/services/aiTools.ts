...
import {
  ...
  runBackupVerification as runBackupVerificationWorkflow
} from '../routes/backup/verificationService';
import type { BackupVerificationType } from '../routes/backup/types';
...
// ============================================
// get_backup_health - Tier 1 (read-only)
// ============================================
...
registerTool({
  ...
  definition: {
    ...
    description: 'Get backup and verification health summary for an organization, with optional device-level detail.',
    input_schema: {
      type: 'object' as const,
      properties: {
        orgId: { type: 'string', description: 'Optional org UUID (must be accessible)' },
        deviceId: { type: 'string', description: 'Optional device UUID for focused results' }
      }
    }
  },
  handler: async (input, auth) => {
    const orgIdInput = typeof input.orgId === 'string' ? input.orgId : undefined;
    const resolved = resolveWritableToolOrgId(auth, orgIdInput);
    ...
    const summary = await getBackupHealthSummary(resolved.orgId);
    const recentVerifications = await listBackupVerifications(
      resolved.orgId,
      deviceId ? { deviceId, limit: 15 } : { limit: 15 }
    );
    const readinessRows = await listRecoveryReadiness(resolved.orgId);
    const readiness = deviceId
      ? readinessRows.find((row) => row.deviceId === deviceId) ?? null
      : null;
...
// ============================================
// run_backup_verification - Tier 2 (mutating)
// ============================================

registerTool({
  tier: 2,
  definition: {
    name: 'run_backup_verification',
    description: 'Run an integrity or restore verification pass for a device and return updated readiness metrics.',
    input_schema: {
      type: 'object' as const,
      properties: {
        orgId: { type: 'string', description: 'Optional org UUID (must be accessible)' },
        deviceId: { type: 'string', description: 'Device UUID' },
        backupJobId: { type: 'string', description: 'Optional backup job UUID' },
        snapshotId: { type: 'string', description: 'Optional snapshot UUID' },
        verificationType: { type: 'string', enum: ['integrity', 'test_restore', 'full_recovery'] },
        highImpactApproved: { type: 'boolean', description: 'Required for full_recovery verification runs' }
      },
      required: ['deviceId']
    }
  },
  handler: async (input, auth) => {
    ...
    const verificationType = (typeof input.verificationType === 'string'
      ? input.verificationType
      : 'integrity') as BackupVerificationType;
    if (verificationType === 'full_recovery' && input.highImpactApproved !== true) {
      return JSON.stringify({ error: 'full_recovery requires highImpactApproved=true' });
    }

    try {
      const result = await runBackupVerificationWorkflow({
        orgId: resolved.orgId,
        deviceId: input.deviceId as string,
        verificationType,
        backupJobId: typeof input.backupJobId === 'string' ? input.backupJobId : undefined,
        snapshotId: typeof input.snapshotId === 'string' ? input.snapshotId : undefined,
        source: 'ai.run_backup_verification',
...
// ============================================
// get_recovery_readiness - Tier 1 (read-only)
// ============================================
...
registerTool({
  ...
  definition: {
    ...
    description: 'Get per-device recovery readiness scores, estimated RTO/RPO, and risk factors.',
    input_schema: {
      type: 'object' as const,
      properties: {
        orgId: { type: 'string', description: 'Optional org UUID (must be accessible)' },
        deviceId: { type: 'string', description: 'Optional device UUID' },
        includeRiskFactors: { type: 'boolean', description: 'Include risk factors in response (default true)' }
      }
    }
  },
  handler: async (input, auth) => {
    ...
    const includeRiskFactors = input.includeRiskFactors !== false;
    const deviceId = typeof input.deviceId === 'string' ? input.deviceId : undefined;

    let rows = await listRecoveryReadiness(resolved.orgId);
    if (deviceId) {
      rows = rows.filter((row) => row.deviceId === deviceId);
    }

    const serializedRows = includeRiskFactors
      ? rows
      : rows.map((row) => ({ ...row, riskFactors: [] }));
