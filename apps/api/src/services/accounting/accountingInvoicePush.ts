 ...
 *
 * All DB access goes through the ambient `db` — this service does not open its
 * own request/system context, matching `accountingMappingService`. Callers
 * (routes, jobs) provide the context; only the QBO network call itself is
 * wrapped in `runOutsideDbContext` so a slow provider round trip never pins a
...
import { AccountingMappingError, resolveConnectionAndToken, syncMappedEntity } from './accountingMappingService';
...
export async function pushInvoiceToAccounting(invoiceId: string, partnerId: string): Promise<InvoicePushOutcome> {
  const { conn, liveConn } = await resolveConnectionAndToken(partnerId, 'quickbooks').catch(translateMappingError);
  ...
  // Currency guard runs BEFORE any org/item lookup or provider call
  // (multi-currency §11 contract, accountingCurrency.ts:143-186).
  try {
  ...
  let orgMapping = orgMappingRows.find((m) => m.breezeEntityId === inv.orgId) ?? null;
  ...
  const itemMappingByItemId = new Map(itemMappingRows.map((m) => [m.breezeEntityId, m]));
  ...
  const mappingRow = await upsertInvoiceMappingPending({
  ...
  try {
    persisted = await persistInvoiceRemoteRef({
...
/** No-op if the invoice was never pushed (no mapping row, or one with no remoteEntityId). */
export async function voidInvoiceInAccounting(invoiceId: string, partnerId: string): Promise<void> {
  const { conn, liveConn } = await resolveConnectionAndToken(partnerId, 'quickbooks').catch(translateMappingError);

  const invoiceMappingRows = await loadMappingRowsForType(partnerId, conn.id, 'invoice');
  const mappingRow = invoiceMappingRows.find((m) => m.breezeEntityId === invoiceId) ?? null;
  if (!mappingRow || !mappingRow.remoteEntityId) return;
  ...
  const mappingSeam: AccountingEntityMappingSeam = {
    remoteEntityId: mappingRow.remoteEntityId,
