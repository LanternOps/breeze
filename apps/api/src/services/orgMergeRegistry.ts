...
const SPECIAL: Record<string, OrgMergePolicy> = {
  ...
  ticket_mailbox_connections: { kind: 'repoint' }, // Gmail rows carry org_id; plain repoint to the survivor preserves the cursor/sub/generation. Safety against a survivor credential that reads a DIFFERENT Google account is enforced at POLL time: every sweep verifies the mailbox's live immutable sub against the stored google_account_sub before ingesting and goes reauth_required on a mismatch (sweepOneGmail). m365 rows carry org_id NULL and never merge.
