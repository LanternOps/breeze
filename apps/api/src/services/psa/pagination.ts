 ...
 * long before this (1000 items / 100 per page = 10 requests); this only fires if
 * a PSA hands back tiny-or-empty pages with a forever-advancing cursor.
 */
export const MAX_PSA_PAGES = 50;
...
export async function collectPaginated<T>(
  ...
  for (let page = 0; page < MAX_PSA_PAGES; page++) {
    ...
    if (items.length >= limit) {
      // More upstream iff we overshot the cap or the provider offered another page.
      return { items: items.slice(0, limit), truncated: items.length > limit || next !== null };
    }
