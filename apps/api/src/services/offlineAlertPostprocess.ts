...
function keys(deviceId: string, ruleId: string) {
  return {
    cooldown: `breeze:alerts:cooldown:${ruleId}:${deviceId}`,
...
export async function readOfflineAlertRedisSuppression(redis: Redis, deviceId: string, rule: OfflineRulePlan) {
  const k = keys(deviceId, rule.ruleId);
  const [cooling, entries, adaptiveRaw] = await Promise.all([redis.exists(k.cooldown), redis.lrange(k.flap, 0, -1), redis.get(k.adaptive)]);
...
if ARGV[4] == '1' and occurred + 1800000 > now then
...
export async function applyOfflineAlertPostprocess(redis: Redis, effect: OfflineEffect): Promise<void> {
  ...
  const k = keys(effect.deviceId, p.ruleId);
