...
async function scanBinaryDir(dir: string): Promise<BinaryInfo[]> {
  ...
  try {
    ...
    console.warn(`[binarySync] Agent binary directory not found: ${dir}`);
    return results;
  }
...
export async function syncBinaries(): Promise<void> {
  ...
  if (versionFile) {
    try {
      ...
      console.warn(`[binarySync] Could not read version file: ${versionFile}`);
    }
  }
  ...
  // Register each binary in the DB
