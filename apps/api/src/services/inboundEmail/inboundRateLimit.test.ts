...
describe('bounded overshoot is accepted under concurrency (design decision, option A)', () => {
  it('two deliveries that both peek before either charges can both be admitted (documented overshoot)', async () => {
    const r = new FakeRedis();
    const now = Date.now();
    // A single partner slot. Two concurrent deliveries each peek FIRST (both see an
    // empty window ⇒ both pass), then each charges after its pipeline commits. The
    // window ends at 2, one over the limit of 1. This is the deliberate, bounded
    // trade accepted in option A: the cap is approximate under a simultaneous burst
    // (bounded by worker concurrency), in exchange for never quarantining a
    // legitimate creation and having no redelivery cap-bypass.
    const limits: InboundCapLimits = { perSenderPerHour: 30, perDomainPerHour: 200, perPartnerPerHour: 1 };
    const checks = () => buildInboundCapChecks('p1', 'jane@acme.com', limits);

    const peekA = await peekInboundThrottle(asRedis(r), checks(), now);
    const peekB = await peekInboundThrottle(asRedis(r), checks(), now); // both peek before any charge
    expect(peekA.throttled).toBe(false);
    expect(peekB.throttled).toBe(false);

    await chargeInboundTickets(asRedis(r), checks(), 'A', now);
    await chargeInboundTickets(asRedis(r), checks(), 'B', now);
    // Overshoot by one — accepted and bounded. The next peek now correctly throttles.
    expect(r.countInWindow('inbound:tix:partner:p1', now - 60 * 60 * 1000)).toBe(2);
