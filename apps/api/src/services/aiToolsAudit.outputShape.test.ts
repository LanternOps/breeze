...
describe('query_audit_log output shape (A-W05 keyset)', () => {
  ...
  const AUDIT_ROW_SHAPE = {
    id: 'id', actorType: 'short', actorEmail: 'short', action: 'short',
    resourceType: 'short', resourceName: 'medium',
  } as const;
  ...
  it('a default page fits, carries the keyset envelope, and replaces details with detailsKeys by default', async () => {
    ...
    expect(out.entries[0]!.detailsKeys).toEqual(['foo', 'baz']);
  ...
  it('includeDetails: true returns the details object', async () => {
    ...
    expect(out.entries[0]!.details).toEqual({ foo: 'bar', baz: 'qux' });
  });
  ...
  it('a nextCursor carries the exact microsecond-precision timestamp text and turns into a (timestamp, id) < (t, i) predicate on the next call', async () => {
    ...
    expect(decoded.t).toBe(rows[19]!.timestampText);
...
describe('query_change_log output shape (A-W05 keyset)', () => {
  ...
  const CHANGE_ROW_SHAPE = {
    id: 'id', changeType: 'short', changeAction: 'short', subject: 'medium',
    hostname: 'short', deviceId: 'id',
  } as const;
  function changeRow(i: number, overrides: Record<string, unknown> = {}) {
    return {
      ...
      afterValue: { version: '2.0', build: 42 },
      details: null,
      ...overrides,
    };
  }
  ...
  it('a default page fits, carries the keyset envelope, total and filters, and replaces values with changedKeys by default', async () => {
    const rows = Array.from({ length: 60 }, (_, i) => changeRow(i));
    dbMock.setRows(rows);
    dbMock.setTotal(60);
    const raw = await tool.handler({}, auth());
    const out = JSON.parse(raw) as {
    ...
    expect(out.changes[0]!.changedKeys).toEqual(['version', 'build']);
  ...
  it('includeValues: true returns before/after/details', async () => {
    ...
    expect(out.changes[0]!.beforeValue).toEqual({ version: '1.0' });
    expect(out.changes[0]!.afterValue).toEqual({ version: '2.0', build: 42 });
  });
