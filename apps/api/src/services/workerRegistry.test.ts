...
// Phase 2 wave P2-6 Task A5, #4193).
...
const EXPECTED_117_NAMES = [
  ...
  'alertVerdictScheduler', 'aiAgentSweepScheduler', 'accountingSyncWorker', 'aiAgentImpactRollup',
];

describe('workerRegistry: losslessness', () => {
  it('contains exactly the 117 known names, in order', () => {
    expect(WORKER_REGISTRY.map((e) => e.name)).toEqual(EXPECTED_117_NAMES);
  });

  it('has exactly 117 entries', () => {
    expect(WORKER_REGISTRY.length).toBe(117);
  });
...
describe('workerRegistry: selectWorkers', () => {
  it("'all' selects every entry", () => {
    expect(selectWorkers('all').length).toBe(117);
  ...
  it("'api' and 'worker' partition the set with no overlap and no loss", () => {
    ...
    expect(api.length + worker.length).toBe(117);
