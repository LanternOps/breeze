...
// Expected ActionExecutionResult outcome per skip reason — the INVERSE of the
...
const EXPECTED_STATUS: Record<AgentRunSkipReason, 'succeeded' | 'failed'> = {
...
describe('executeAiTriageAction', () => {
  ...
  it('maps a cooldown skip to a succeeded outcome with an info log', async () => {
  ...
  it('maps ownership_mismatch to a failed outcome', async () => {
  ...
  it('maps device_not_in_org to a failed outcome', async () => {
  ...
  it('every AgentRunSkipReason is classified and produces a recorded log', async () => {
    for (const [reason, expectedStatus] of Object.entries(EXPECTED_STATUS) as Array<[
      AgentRunSkipReason,
      'succeeded' | 'failed',
    ]>) {
      ...
      expect(actual.deviceId).toBe('dev-1');
      expect(result.outcome.status, reason).toBe(expectedStatus);
