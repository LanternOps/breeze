...
const TOOL_PERMISSIONS: Record<string, { resource: string; action: string } | Record<string, { resource: string; action: string }>> = {
  ...
  get_network_device_configs: { resource: 'devices', action: 'read' },
  get_network_firmware_status: { resource: 'devices', action: 'read' },
  backup_network_config: { resource: 'devices', action: 'write' },
...
const TOOL_RATE_LIMITS: Record<string, { limit: number; windowSeconds: number }> = {
  ...
  backup_network_config: { limit: 10, windowSeconds: 600 },
