...
/**
 * `org.merge.failed` fires from executeOrgMerge's own catch block, BEFORE
 * anything is committed: the loser has just been unfenced back to its prior
 * status, so — unlike `org.merge.completed`/`org.merge.shell_stamp_failed`,
 * which are deliberately org-less because the loser is about to be erased —
 * neither org here is going anywhere. Written org-less, this failure was
 * invisible in every org-scoped audit view. `routes/orgMerge.ts`'s
 * `org.merge.requested` is the sibling that already attributes a merge event
 * to a live org (`orgId: survivorId`) — mirror it.
 */
describe('executeOrgMerge failure path (org.merge.failed audit)', () => {
  const LOSER = '77777777-7777-7777-7777-777777777777';
  const SURVIVOR = '88888888-8888-8888-8888-888888888888';
  ...
  it('attributes org.merge.failed to the survivor org, not org-less', async () => {
    const error = vi.spyOn(console, 'error').mockImplementation(() => {});
    vi.spyOn(orgMergeModule, 'loadAndValidate').mockResolvedValue({ loser, survivor });
    vi.spyOn(orgMergeModule, 'fenceLoser').mockResolvedValue(undefined);
    vi.spyOn(orgMergeModule, 'assertPairStillMergeable').mockRejectedValue(
      new Error('survivor moved mid-drain'),
    );
    const unfence = vi.spyOn(orgMergeModule, 'unfenceLoser').mockResolvedValue(undefined);

    await expect(orgMergeModule.executeOrgMerge(input)).rejects.toThrow('survivor moved mid-drain');

    expect(unfence).toHaveBeenCalledWith(loser);
    expect(createAuditLog).toHaveBeenCalledTimes(1);
    expect(vi.mocked(createAuditLog).mock.calls[0]![0]).toMatchObject({
      // The merge failed before anything committed — both orgs are still live,
      // so this must NOT be org-less like the post-commit merge events.
      orgId: SURVIVOR,
      ...
      details: expect.objectContaining({
        ...
        loserOrgName: 'Loser Co',
        survivorOrgId: SURVIVOR,
        error: 'survivor moved mid-drain',
      }),
    });
