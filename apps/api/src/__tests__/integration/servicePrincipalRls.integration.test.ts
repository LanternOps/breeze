...
async function insertKey(input: {
  ...
  const [key] = await withSystemDbAccessContext(() =>
    db
      ...
      .values({
        partnerId: input.partnerId,
        servicePrincipalId: input.principalId,
        name: input.name,
...
describe('service-principal database contract', () => {
  ...
  runDb.each([
    ...
    const cause = await captureCause(() =>
      (getTestDb() as any).insert(servicePrincipals).values({
        partnerId: partnerA.id,
    ...
    expect(cause?.constraint_name).toBe('service_principals_scopes_check');
  });
  ...
  runDb('rejects unknown principal and key status values', async () => {
    ...
    expect(principalStatusCause?.constraint_name).toBe('service_principals_status_check');
    ...
    const keyStatusCause = await captureCause(() =>
      (getTestDb() as any).insert(servicePrincipalKeys).values({
        partnerId: partnerA.id,
    ...
    expect(keyStatusCause?.constraint_name).toBe('service_principal_keys_status_check');
  });
  ...
  runDb('enforces principal ownership and rotated-key lineage within one partner', async () => {
    ...
    const wrongPrincipalCause = await captureCause(() =>
      withDbAccessContext(contextA, () =>
        db.insert(servicePrincipalKeys).values({
          partnerId: partnerA.id,
          servicePrincipalId: principalB.id,
    ...
    expect(wrongPrincipalCause?.constraint_name).toBe(
      'service_principal_keys_principal_partner_fk',
    );
    ...
    const wrongLineageCause = await captureCause(() =>
      withDbAccessContext(contextA, () =>
        db.insert(servicePrincipalKeys).values({
          partnerId: partnerA.id,
          servicePrincipalId: principalA.id,
    ...
    expect(wrongLineageCause?.constraint_name).toBe(
      'service_principal_keys_rotated_from_partner_fk',
    );
  });
});

describe('service-principal partner-axis RLS (breeze_app)', () => {
  ...
  runDb('hides Partner B principals and keys from Partner A', async () => {
    ...
    const [principals, keys] = await withDbAccessContext(contextA, () =>
      Promise.all([
        db.select({ id: servicePrincipals.id }).from(servicePrincipals)
          .where(eq(servicePrincipals.id, principalB.id)),
        db.select({ id: servicePrincipalKeys.id }).from(servicePrincipalKeys)
          .where(eq(servicePrincipalKeys.id, keyB.id)),
      ]),
    );
  ...
  runDb.each([
    { table: 'service_principals' as const },
    { table: 'service_principal_keys' as const },
    ...
    const cause = await captureCause(() =>
      withDbAccessContext(contextA, () =>
        table === 'service_principals'
          ? db.insert(servicePrincipals).values({
              partnerId: partnerB.id,
  ...
  runDb.each([
    ...
    const cause = await captureCause(() =>
      withDbAccessContext(contextA, () =>
        table === 'service_principals'
          ? db.update(servicePrincipals).set({ partnerId: partnerB.id })
              .where(eq(servicePrincipals.id, principalA.id))
          : db.update(servicePrincipalKeys).set({ partnerId: partnerB.id })
              .where(eq(servicePrincipalKeys.id, keyA.id)),
      ),
    );
