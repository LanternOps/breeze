...
function deriveMaterialTables(migrations: readonly string[]): Set<string> {
  ...
  for (const migration of migrations) {
    ...
    for (const loop of sql.matchAll(/\bFOREACH\s+\w+\s+IN\s+ARRAY\s+ARRAY\s*\[([^\]]*)\]\s*LOOP\b([\s\S]*?)END\s+LOOP/gi)) {
      if (!new RegExp(String.raw`CREATE\s+TRIGGER\b[^;]*?${MATERIAL_FUNCTION}`, 'i').test(loop[2]!)) continue;
    ...
    const trigger = new RegExp(String.raw`CREATE\s+(?:OR\s+REPLACE\s+)?TRIGGER\s+${IDENT}\s+[^;]*?\bON\s+${TARGET}\s+[^;]*?${MATERIAL_FUNCTION}`, 'gi');
 ...
 */
function withLockScope(sql: string, axis: 'partners_shared' | 'orgs_exclusive'): string {
...
function findUnlockedWrites(sql: string, tables: ReadonlySet<string>): string[] {
  ...
  const partners = analyzeMigrationDml(withLockScope(sql, 'partners_shared'));
  const orgs = analyzeMigrationDml(withLockScope(sql, 'orgs_exclusive'));
  const code = maskSql(sql, true);
