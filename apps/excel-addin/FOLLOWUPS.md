...
- [ ] **PowerPoint add-in** — ≈ Word, now mostly additive (server is host-keyed, the core shell + `HostAdapter` seam are proven): populate `CLIENT_TOOL_REGISTRIES.powerpoint` + a prompt + `apps/powerpoint-addin` shell. PPT's Office.js API is less mature (some ops need an OOXML fallback).
