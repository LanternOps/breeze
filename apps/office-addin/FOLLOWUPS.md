...
- [ ] **Physical package split (Phase 3)** — extract `packages/office-addin-core` + rename `apps/office-addin` → `apps/excel-addin`. The logical seam already prevents core drift; this hardens the dependency boundary before the 2nd host. Roadmap in the Phase-1/2 plan; expand into its own plan before executing.
- [ ] **Word add-in (Phase 4)** — once the core is split: populate `CLIENT_TOOL_REGISTRIES.word` + a Word system prompt (server is already host-keyed, so `host:'word'` becomes creatable automatically), a Word `HostAdapter`, and a manifest. Word tools (insert/format text, find/replace, comments, content controls).
