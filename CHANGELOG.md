...
### Added
- Three business report types under **Reports → Templates → Business**: **Ticket SLA attainment** (response and resolution attainment, recalculated from each ticket's own timestamps so a late-but-answered ticket still counts as a miss), **Technician time & billability** (utilization against an assumed weekly capacity, the billable / included / non-billable split, and how much billable time has actually been billed), and **AR aging** (unpaid balances bucketed current / 1–30 / 31–60 / 61–90 / 90+ days past due, one set of totals per currency, no conversion). Each is created as either an organization report or a partner-wide report — a partner-owned report covers every active or trial organization the MSP manages, resolved live at each run. Every business report needs a partner login holding the type's permissions (`tickets:read`, `time_entries:read` + `tickets:read`, or `invoices:read`); a partner-owned report additionally needs access to all organizations. All three are internal to MSP staff: they never appear in a customer portal and deliver only to email addresses, never customer contacts. Two approximations are disclosed on the report itself (paused ticket time is a lifetime total, so a pause after first response slightly flatters response attainment; technician capacity is assumed uniform, so PTO and part-time schedules are not modeled). No new environment variables; no action needed by self-hosters (#3198).

### Removed

- Legacy policy **Alerts** and **Service & Process Monitoring** tabs; the old `/alerts/rules` page redirects to Monitors. Legacy alert-rule and alert-template write endpoints (including tests and toggles) return `410 Gone`; historical reads remain.
- The legacy policy evaluator (`evaluateDeviceAlertsFromPolicy`), `alert_rule`/`monitoring` configuration feature types (retained only as `RETIRED_CONFIG_FEATURE_TYPES`), `manage_alert_rules.list_templates`, and `scripts/migrateToConfigPolicies.ts`. Use monitor definitions and policy monitor attachments.
...
- **Complete conversion on the previous release before upgrading.** Run **Convert everything** and review refusals while previews and Undo are available. This release must follow a full conversion release cycle. Its unattended sweep runs after the API starts serving requests, leaving unconverted conditions without evaluation until it finishes. Unconvertible sources are retired and listed for review; network checks are excluded from automatic retirement. `BREEZE_LEGACY_ALERTING_SWEEP=false` skips conversion, but the unretired rule/watch count still runs and reports non-zero counts to logs and Sentry.
- **Migration `2026-10-31-100000-legacy-alerting-retirement-sweep.sql`** re-keys policy check intervals to the `monitors` link and reports unretired counts. It is idempotent and deletes no data. Review the retirement banner and any failed partner conversions after startup; recreate needed conditions as monitors.
