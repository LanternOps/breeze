...
**Two traps when scoping a run to one file.** `apps/api`, `apps/web`, `packages/shared`, and most other workspace packages define `"test": "vitest"` (bare, watch-mode by default) — the `--` trap below hits these hardest, since it leaves them stuck in watch mode over the whole suite. A few packages (`apps/portal`, `apps/helper`, `apps/mobile`, `packages/extension-sdk`, `packages/extension-web-sdk`, `packages/extension-testkit`) already define `"test": "vitest run"`, which forces run-once mode regardless — for those, the same `--` still gets forwarded but is less harmful (it just adds a spurious no-op filter). Check `grep '"test"' <pkg>/package.json` before assuming either shape.
