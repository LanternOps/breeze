...
|---|---|---|
| `GET /backup/health?orgId=&source=&health=&status=&withBackup=&search=&cursor=` | `backup:read`; `orgId` optional — omitted = all accessible orgs (`auth.orgCondition`, the devices-list pattern) | rows + summary for the overview |
