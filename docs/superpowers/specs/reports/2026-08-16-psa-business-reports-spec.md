...
**CHECK constraints.** Both `*_execution_scope_shape_chk` constraints gain a fourth branch: `execution_scope_kind = 'partner_wide' AND execution_scope_site_ids IS NULL AND execution_scope_user_id IS NOT NULL AND execution_scope_user_id = requested_by AND execution_scope_principal_kind = 'user'` (on `reports`, `= created_by`). The RLS coverage suite's execution-scope projection test (`siteScope.projections.test.ts`) is unchanged: no new column, only a new admitted value.
...
**Merge/erasure registries.** `reports` is `kind:'custom'` in `orgMergeRegistry` — the merge pass only touches rows whose `org_id` is the loser org, so partner-owned rows are untouched (intended; they aggregate across orgs). `report_runs` in `tenantCascade.ts` reaches its org through `reports.org_id`; partner-owned runs are not org data and survive org erasure. Partner deletion: verify the partner cascade path deletes partner-owned reports and, with the `report_runs` FK moved to `ON DELETE CASCADE` (§4), their runs.
...
| *(2026-09-21)* `reports_execution_scope_shape_chk` / `report_runs_execution_scope_shape_chk` gain the `partner_wide` branch (§3.1a) | `siteScope.test.ts` encode/decode round-trip for the new kind; forge a `partner_wide` row with `principal_kind = 'system'` → 23514 |
