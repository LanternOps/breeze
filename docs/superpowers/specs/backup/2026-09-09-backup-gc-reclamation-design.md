...
|---|---|
| `2026-10-15-150002-backup-jobs-base-pin-and-storage-identity.sql` | `backup_jobs.base_snapshot_id`, `publish_lease_expires_at`, `storage_identity`, partial index; `backup_snapshots.storage_identity` (nullable, no backfill — §3.6 self-heal); DDL only, no `breeze.scope` needed |
| `2026-10-15-150003-backup-snapshot-retirements.sql` | table + RLS (shape 1, enable+force, four policies) + indexes |
