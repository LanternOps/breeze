...
7. **Phase 2 re-reads under the invoice lock.** The QBO round trip is not covered by any lock, so after create the coordinator locks the invoice and re-reads the mapping and the payment: payment row gone (voided/refunded during the call) → keep the mapping, stamp the remote ref, flip `pending_op = 'delete'`, re-enqueue; invoice now void → same; echo already adopted the row with the same remote id → keep the newer token, do not overwrite; amount changed (partial refund) → stamp the remote ref and set the mapping `error` per decision 9. Job ids are `accounting-payment-<mappingId>-<op>` so a delete enqueued while a push job is still active is never swallowed by a deterministic id.
...
12. **Delete semantics.** `POST payment?operation=delete {Id, SyncToken}`. Exact "Object Not Found" / already-deleted → success. "Stale object" means the Payment still exists with a newer token: read it once, retry the delete once with the fresh token, then fail (retryable). Success removes the mapping row.
...
- `InvoiceDetail.tsx`: payment rows show an "In QuickBooks" badge when `accountingSync.status === 'synced'`, a warning badge with `lastError` when `error`, and a muted "Syncing…" when pending. The void button stays for Breeze-origin rows (source `manual` / `stripe`) and stays hidden for `quickbooks`.
