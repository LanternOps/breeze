...
- `addContractLine` (`invoiceService.ts:347-453`) returns `{ line, pricedFrom }`. W04 already changes generation to destructure `line` (it needs `baseLine.taxable` / `baseLine.costBasis` for the overage leg). So the invoice-line id evidence needs is **already in hand** after W04, at no extra cost.
- `addContractLine` opens `db.transaction(...)` internally (`:361`). `db` is the AsyncLocalStorage-routed proxy (`db/index.ts:752-754`, `:792-794`), so inside `generateDueInvoice`'s ambient transaction that is a savepoint, not a second connection — evidence written by the outer scope commits or rolls back with the lines.
...
- W04 produces `overages: OverageSummary[]` — `{ contractLineId, description, counted, included, overage, mode }` — for **both** modes, and `applyAllowance` returns `{ counted, billed, included, overage, overageMode }`. The overage invoice line is a **sibling** (`parentLineId: null`) sharing `sourceId` with its base line (W04 decision 8), so base and overage rows are distinguished by their own `invoice_lines.id` and never by lineage columns.
