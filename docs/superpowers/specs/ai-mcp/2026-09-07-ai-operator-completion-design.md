...
Verified against the baseline commit above; issue states were checked on GitHub on 2026-09-07. Citations were re-checked on origin/main b3d528d869 the same day; the 16 commits since the baseline touch no docs or migrations. Phase 1 (#3821), Phase 2 (#4187 and #4188–#4193), and the builder (#5048) are closed. Old “awaiting review” headers and unchecked task boxes are not reliable indicators of remaining work.
...
| The intent outbox has decision events, but no durable completed/failed event | apps/api/src/db/schema/actionIntents.ts (event types `intent_approved`, `intent_rejected`, `intent_expired`, `intent_cancelled`; outbox `event_kind IN ('requested','result')`); apps/api/src/jobs/intentReleaseWorker.ts:1291 accepts only those four | Add reliable terminal-result signals and reconciliation |
...
| Every `every:` repeatable is epoch-aligned | apps/api/src/jobs/scheduleRegistry.ts header comment and lane table | Coordinator and publisher register on a lane, never a bare `every: 24h` (§11.2) |
...
Existing patch scheduler occurrences produce or adopt one task/job lineage when Operator management is enabled. Do not enqueue the ordinary patch job and an additional Operator-created job for the same occurrence. Enable/disable changes affect future occurrences; they never duplicate/replay an already-owned one.
...
- A successful script exit is execution evidence; it is insufficient for service/disk/application recovery without the recipe's endpoint criterion.
...
| ai_operator_task_outbox | id, org_id, task_id, source identity/transition, due_at, published_at, attempts | Shape 1; unique org/task/source/transition; replay-safe scheduler delivery |
...
Retention: use the approved AI/audit retention policy, with separately bounded checkpoint text and longer permitted outcome metadata. Define each field's included/excludedSensitive/excludedOpen export classification; “JSON” alone is not justification to omit customer-authored objectives/runbooks. Erasure also removes queued wakeups and prevents re-creation. Raw input redaction work (#4181) is a prerequisite to exposing additional trace content, not permission to copy raw outputs into task events.

Migrations must be new, idempotent, date-prefixed, correctly ordered, and contain no inner transaction blocks. Do not edit shipped migrations. Inspect existing filenames before choosing the prefix: the newest shipped migration is named `2026-10-13-…`, more than five weeks ahead of the authoring date, so a file named for today would replay before it.

### 11.1 Indexes and read paths

Every new table is read under forced RLS as `breeze_app`, where only leakproof operators (`uuid_eq`, `texteq`, timestamp comparisons, `IS NULL`) can become index conditions. jsonb `->>`, `LIKE`, and enum equality cannot, and one leaky arm turns an entire `OR` into a post-policy filter. State and phase columns are therefore `text` with CHECK constraints, never `pgEnum`, and partial-index predicates are stated verbatim as constants in the query (a Drizzle `sql` template inlines; `eq()` binds). Required in the creating migration:
...
- **Scheduling:** register both in `apps/api/src/jobs/scheduleRegistry.ts` on a free lane. No bare `every: 24h`; BullMQ aligns those to the Unix epoch, so every such job fires together at 00:00 UTC (see the file's header comment). Task deadlines add ±10 % jitter so the 72-hour default does not create an expiry wave.
...
- **Phase 4 — fleet operations:** executor readiness, canary rollout, scheduled child tasks. Patch maintenance is sequenced last among execution-expanding waves (plan §9, decision D1).
...
- 2026-09-07: Codex `xhigh` quorum on the revised draft: QUORUM_PLACEHOLDER
