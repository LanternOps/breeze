...
| Service restart is a device command, not a script execution | apps/api/src/services/aiToolsScripts.ts:649 (`executeCommand`, `restart_service`) | The thin slice's execution reference is the device command id; the result is observed from command completion (§6.5, plan P3-1) |
...
Concretely, the operation identity is reserved on the intent itself: nullable `task_id`, `task_step_key`, and `operation_key` columns on `action_intents` (all null or all set, by CHECK), a partial unique index on `(org_id, task_id, operation_key)` over live statuses, and a permanent copy on the operation row. The default run-scoped idempotency key (intentService.ts:1182) stays for legacy runs. A task-linked intent uses the task-scoped key, so a continuation run that re-proposes the same operation attaches to the existing intent instead of minting a second one, and the "explicit key reused by another run" rejection (intentService.ts:1527) is relaxed only for same-task runs. P3-2 adopts these rows by reference when `ai_operator_operations` gains its full shape; it never recreates an intent or changes an execution key.

For the thin slice, the supported service restart is a device command (aiToolsScripts.ts:649), so the execution reference is the device command id and the domain result is the command's completion record. Script executions, patch job targets, playbook executions, ticket comments, and report deliveries are the other reference kinds; each resolves through its adapter with an authorization check at read time (§11.3).
