...
- Tenant part: enabled `tool_source_tools` whose source is `active` and whose owner is visible to `auth`: the caller's org, plus partner-wide sources of the caller's partner. **Amendment (W1 plan):** org tokens read partner-wide rows through an additive `FOR SELECT`-only policy `<table>_partner_wide_select USING (org_id IS NULL AND partner_id = public.breeze_current_partner_id())` on both tables (Partner-Wide First step 3; template `2026-10-05-110000-config-policy-partner-wide-select.sql`), so resolution runs inside the caller's own RLS context with the app-layer predicate `org_id = :org OR (org_id IS NULL AND partner_id = :partner)`; no system-context escalation. Org admins see inherited partner-wide sources read-only.
...
**Tables and shapes.** `tool_sources`, `tool_source_tools`, `flows` are dual-owned (org XOR partner, `_one_owner_chk`, one dual-axis policy `system OR breeze_has_org_access(org_id) OR breeze_has_partner_access(partner_id)`, plus a separate additive `FOR SELECT`-only `<table>_partner_wide_select` policy keyed on `breeze_current_partner_id()` so an org token can read its own partner's partner-wide rows without widening writes — main's Partner-Wide First step 3). `flow_runs`, `flow_run_steps` have `org_id NOT NULL` (shape 1). Register all five in `DUAL_AXIS_TENANT_TABLES` / auto-discovered shape-1 as appropriate in `rls-coverage.integration.test.ts`.
...
**Migrations.** Named to sort after the newest committed file on origin/main (`2026-10-13-110000-…` as of 2026-09-07; the ratchet runs ~5 weeks ahead of real time), idempotent, policies in the same file as the table, `set_config('breeze.scope','system')` form for any DML, run `migrationRlsScope.test.ts`.
