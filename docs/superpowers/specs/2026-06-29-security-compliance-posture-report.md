...
**Date:** 2026-06-29
**Related:** composes with `docs/superpowers/plans/2026-06-29-downloadable-report-runs.md` (snapshot/download plumbing)
...
**CIS is a bonus path, not a dependency.** CIS baseline scans are not yet routinely run in the target fleets, so **every primary control percentage derives from `security_status`, `device_patches`, the integration tables, and the PAM/elevation tables** — all of which populate without any CIS scan. `cisAvgPassRate` averages per-device pass-rate over devices that have at least one `cis_baseline_results` row; when none exist it is `null` and the PDF renders the hardening section as "Not yet assessed — enable CIS baseline scans to populate," never as 0% (which would read as a failing control rather than an unmeasured one). When CIS coverage later grows, this section lights up with no report changes.
