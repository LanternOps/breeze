...
- `createRemediationRun` re-validates every requested device NOW under the caller's auth: member of finding, org in `accessibleOrgIds` AND matching the finding's own `orgId` (a device-move-trigger re-tenanted membership row can otherwise point at a device whose live org has moved out from under the finding), site in `allowedSiteIds` (when restricted), not decommissioned, not ephemeral. Failures become `skipped` targets with `skip_reason` (`site_denied`/`not_member`/`decommissioned`) — never silently dropped. Snapshot hostname/site into target rows. `actionKind: 'command'` accepts an explicit `commandType` allowlist: `['restart_service','reboot']` — reject anything else (400); `clear_temp_files` is excluded (no single fire-and-forget command primitive — the real disk-cleanup tool needs a filesystem snapshot plus a caller-selected path set). `actionKind: 'script'` takes a `scriptId` instead of a `commandType`, dispatched as `CommandTypes.SCRIPT`.
...
- `deployment_device_id` from the spec sketch dropped: v1 dispatches exclusively via `queueCommandForExecution` (scripts dispatch as `CommandTypes.SCRIPT`, i.e. commandType `'script'`), so only `device_command_id` exists. Deployments remain the path for software installs, out of v1 picker scope.
