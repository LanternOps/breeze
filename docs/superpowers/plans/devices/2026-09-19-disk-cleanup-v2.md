...
W01 → W02 → W03 → W04 → W05 are serial. W01 and W04 change agent code and ship in the next agent release; W01's API half is safe against old agents (spec §3), W04's is gated on `MIN_AGENT_VERSION_SYSTEM_CLEANUP`. Advisor quorum: an adversarial Opus pass (spec §13) is recorded; the Codex xhigh pass was blocked by the subscription's usage limit on 2026-09-19 and must be run and recorded before W01 starts implementation.
...
7. Migration names re-checked against `origin/main` on 2026-09-19: `150000`/`150100` sort last. Re-check at commit time.
