...
Add a status note at the top of `internal/completed/BE-25-usb-peripheral-control.md` stating that enforcement (block/read-only) shipped for Windows via registry + Config Manager (no kernel driver), per plan `docs/superpowers/plans/2026-06-24-usb-blocking-enforcement.md`, and that macOS/Linux remain alert-only.
