...
| D2 amendment (2026-09-23, #6691) | **Amended 2026-09-23 (#6691): category/priority changes restamp while `first_response_at IS NULL` and status ∉ {resolved, closed}; `field_provenance = 'user'` targets survive.** Both write paths restamp (`updateTicketFields` and the AI-triage `applyAiFieldUpdates`) through the shared `resolveEffectiveSlaTargetsForTicket` helper that `createTicket` uses. An explicit SLA value in the same PATCH wins and takes `user` provenance; restamped values are not stamped `user`. Deadlines stay anchored to `created_at`; `sla_breached_at` / `sla_breach_reason` are not touched. | Email, Outlook add-in and AI-chat tickets arrive uncategorised and stayed on the default SLA forever. Before the first response the customer has seen no commitment, so moving it is safe; the index-backed-SQL rationale above still holds because this restamps the columns rather than resolving the chain per query. |
...
(Amended 2026-09-23, #6691: they now DO restamp while `first_response_at IS NULL` and status ∉ {resolved, closed}, skipping `field_provenance = 'user'` targets — see the D2 amendment row.)
