...
**Interfaces:**
- Produces: `issueInvoice` performs ALL validation inside its system transaction in this exact order (deadlock-safe lock ordering; matches Task 9 and stripeReconcile):
  1. `SELECT * FROM invoices WHERE id = $1 FOR UPDATE` — recheck `status = 'draft'` on the LOCKED row (`NOT_A_DRAFT` if lost the race). The pre-tx `assertDraft` stays as a fast-fail but is no longer authoritative.
  2. Lock lines: `SELECT * FROM invoice_lines WHERE invoice_id = $1 ORDER BY id FOR UPDATE`.
  3. Lock ALL referenced source rows with **no billing-status filter** (this is the inverted-predicate fix at `:663/:667`): `inArray(timeEntries.id, timeIds)` ordered by id `.for('update')`, then assert on the locked rows: every id exists, `billingStatus === 'not_billed'` (`SOURCE_ALREADY_BILLED` otherwise), org matches the invoice org. Same for `ticketParts`. Contract-sourced lines: assert `contract.currencyCode === inv.currencyCode` (Task 5's guard, now under lock).
  4. Compute totals with `computeInvoiceTotals(lineRows, taxRate, inv.currencyCode)` (already currency-aware), allocate the invoice number (existing in-tx allocator at `:677-684` is unchanged), UPDATE to issued **without touching `currencyCode`** (Task 2), flip source rows.

- [ ] **Step 1: Write the failing race test** — two drafts referencing the SAME `not_billed` time entries (forge recipe from `invoiceService.issue.integration.test.ts:118-140`, now stamping currency per Task 6); two dedicated clients issue concurrently with a barrier; assert: exactly one succeeds, the loser gets `SOURCE_ALREADY_BILLED` **after** lock-wait (use `waitForBackendLockWait`), sources billed exactly once, exactly one invoice number consumed. Also a same-invoice double-issue case: both clients issue THE SAME draft → one wins, loser `NOT_A_DRAFT`.
- [ ] **Step 2: Run to verify failure** — current code double-issues (both pass the empty billed-check).
- [ ] **Step 3: Implement** the transaction reorder above. Drizzle idiom: `.for('update')` (already used at `:663`); keep every read that feeds validation inside the tx — delete the pre-tx line read at `:644` or demote it to fast-fail only.
- [ ] **Step 4: Run** the race test + `invoiceService.issue.integration.test.ts` + `quoteAcceptService` suites (its inline-issue path bypasses `issueInvoice` by design — must stay green untouched) → PASS.
- [ ] **Step 5: Commit** — `fix(api): race-safe issueInvoice — lock invoice, lines, and source rows before validating (B10) (#3774)`.

### Task 9: Race-safe payment recording (manual + Stripe reconcile)

**Files:**
- Modify: `apps/api/src/services/invoiceService.ts:736-747` (`recomputeInvoiceStatus`), `:749-794` (`recordPayment`)
- Modify: `apps/api/src/services/stripeReconcile.ts:55-85` (same invoice-lock discipline)
...
**Interfaces:**
- Produces: `recordPayment` runs one transaction: lock invoice FOR UPDATE → recheck payable status → recompute balance from `invoice_payments` in-tx (consistent because every payment writer locks the invoice first) → **re-evaluate the wave-1 representability guard against the in-tx balance** (the exact-balance-payoff escape must compare against the LOCKED balance, or a racing payment turns a stale "exact payoff" into an admitted non-representable residue) → overpay check → insert → recompute status in the same tx. `stripeReconcile.recordStripePayment` takes the same invoice row lock before its currency/overpay checks and status recompute (its mapping-row idempotency is unchanged).
...
- [ ] **Step 3:** Push, open PR: title `feat: multi-currency wave 2 — document correctness (#3774)`, body summarizes B1/B2/B3/B10 fixes + the drop-default contract, ends with `Closes #3774`. PR targets `main` (not stacked), so Integration Tests run on the PR itself.
- [ ] **Step 4:** One review round (per repo policy), then merge on green.
