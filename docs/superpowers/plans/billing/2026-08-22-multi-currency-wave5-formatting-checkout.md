...
- **Lock order untouched.** The `issueInvoice` locale stamp is a pure key addition to the existing guarded `.set({...})` using the `partner` row already read inside the transaction; never read `stripe_connect_accounts` inside `issueInvoice` (partner-axis row, RLS-invisible under org scope, new lock class). Stripe refresh runs via `runOutsideDbContext` + its own short `withSystemDbAccessContext` (the `partnerStripe.ts:101-110` pattern) — no pooled connection held across the Stripe HTTP call.
...
- PDF width headroom: Intl prefix-code output (`CHF 888,888.88` ≈ 73pt at Helvetica 10) exceeds the taxed `colNumW = contentWidth * 0.15` (≈ 74pt on A4, < 2pt margin). Rebalance the taxed layout to `colDescW 0.44`, `colQtyX 0.46`, `colTaxX 0.64`, `colAmtX 0.83`, `colNumW 0.17`; untaxed stays `0.18` / `0.62` / `0.80` / `0.60`. The test in Step 1 is the arbiter — tune until it passes.

- [ ] **Step 1: Write failing tests** in `invoicePdf.test.ts`: (a) `renderInvoiceHtml` with `currencyCode: 'EUR'`, `documentLocale: 'de-DE'` contains `'1.000,00 €'` (fixture total `1000.00`); (b) `documentLocale: null` + `branding.locale: 'fr-FR'` renders French formatting (`'1 000,00 €'`); (c) `buildInvoiceEmailAmounts({ …, currencyCode: 'JPY', documentLocale: 'en' })` → `total === '¥1,000'`; (d) width: build a `PDFDocument({ size: 'A4', margin: 50 })`, `doc.font('Helvetica').fontSize(10)`, assert `contentWidth * 0.17 >= doc.widthOfString(formatMoney(888888.88, 'CHF', 'de-CH')) + 2` and the same for the untaxed `0.18` (mirror the `quotePdf.test.ts:125-135` measurement style). Existing USD/`$` assertions (`:92-94,:107,:178-200`) must stay green unchanged — Intl `en-US` USD output is byte-identical.
- [ ] **Step 2: Run** `pnpm --filter @breeze/api test invoicePdf` → new cases FAIL.
- [ ] **Step 3: Implement** — delete the private formatter, `import { formatMoney } from '@breeze/shared'`, `import { resolvePartnerDocumentLocale } from './documentLocale'`, thread `locale` as specified, rebalance the taxed columns (update the `:280-282` comment to say the money columns are sized for prefix-code currencies). Keep `InvoiceRow` typed from the schema so `documentLocale` is available without a cast.
...
**Files:**
- Modify: `apps/api/src/services/quotePdf.ts:32-37` (`formatMoney` export → delegating, new signature), `:84-95` (`QuotePdfBranding.locale`), `:97-103` (`QuoteHeader.documentLocale`), `:211-242` (`columnsFor`), `:435,:438,:442,:458-460,:532-534,:594` (call sites), `:772` (`renderQuotePdf` locale)
...
- Column headroom (A4 is the narrower page; `contentWidth = 495.28`): money cells draw with `lineBreak: false`, so overflow overprints. Rebalance `columnsFor`:
  - taxed: `colQtyW 0.07`, `colDescX 0.08`, `colDescW 0.40`, `colUnitX 0.49`, `colNumW 0.155`, `colTaxX 0.655`, `colAmtX 0.82`, `colAmtW 0.18`, `colSummaryAmtX 0.76`, `colSummaryNumW 0.24`
  - untaxed: `colQtyW 0.07`, `colDescX 0.08`, `colDescW 0.50`, `colUnitX 0.59`, `colNumW 0.16`, `colAmtX 0.80`, `colAmtW 0.20`, `colSummaryAmtX 0.76`, `colSummaryNumW 0.24`
  The widened summary box must still leave the summary label box wide enough for its widest bold-12 label — the existing assertion in the same test guards it. The Step 1 measurements are the arbiter; if `CHF 1,000,000.00` at Helvetica-Bold 14 still exceeds `colSummaryNumW`, widen to `0.26`/`0.74` rather than shrinking the font.

- [ ] **Step 1: Extend the width test** (`quotePdf.test.ts:125-135`): alongside the `$` strings assert, for both layouts, `colNumW >= widthOf(formatMoney(888888.88, 'CHF', 'de-CH')) + 2`, `colAmtW >= widthOf(formatMoney(888888.88, 'CHF', 'de-CH') + '/mo') + 2`, and at Helvetica-Bold 14 `colSummaryNumW >= widthOf(formatMoney(1000000, 'CHF', 'de-CH')) + 2`; keep `colAmtX + colAmtW ≈ right` and `colSummaryAmtX + colSummaryNumW ≈ right`. Add a render test: quote fixture with `currencyCode: 'EUR'`, `documentLocale: 'de-DE'`, a line `12000.00` → positioned text contains `'12.000,00 €'`; and `documentLocale: null` + `branding.locale: 'pt-BR'` → `'R$ 12.000,00'`.
...
### Task 6b: `contractTemplateRender` totals use the shared formatter with the quote's locale

**Executor:** codex

**Files:**
- Modify: `apps/api/src/services/contractTemplateRender.ts:26` (import), `:277-298` (`resolveAutoVariables`)
- Test: `apps/api/src/services/contractTemplateRender.test.ts:130-145` (extend)

**Interfaces:**
- Consumes: `formatMoney(value, currency, locale)` from `@breeze/shared`; `resolvePartnerDocumentLocale` is NOT available here (the resolver is pure and takes only the quote row) — use `quote.documentLocale ?? 'en'`. Every caller (`loadContractPdfInputs`, `renderContractBlocksForClient`, `loadContractBlockRenderData`) passes a quote row that is either stamped (sent quotes — the only ones the portal/public routes serve) or a draft preview, where `'en'` matches today's output exactly.
- Produces: `resolveAutoVariables(quote: QuoteRow, opts?)` unchanged signature; body computes `const locale = quote.documentLocale ?? 'en';` and formats `totals.one_time/monthly/annual/total` with `formatMoney(x, currency, locale)`. Change the `:26` import to `import { formatDate, contractUploadedMarker, type ContractPdfBlockData } from './quotePdf';` + `import { formatMoney } from '@breeze/shared';` (the `quotePdf.formatMoney` export becomes a shim with the same `(amount, currency, locale?)` signature — either import compiles; prefer the shared one so this file has no renderer dependency for money). Update the `:277-279` doc comment accordingly.
- `QuoteRow` is `typeof quotes.$inferSelect` (`:53`) and carries `documentLocale: string | null` after the schema change.

- [ ] **Step 1: Write failing tests** — fixture quote with `currencyCode: 'EUR'`, `documentLocale: 'de-DE'`, `oneTimeTotal: '810.00'` → `'totals.one_time' === '810,00 €'`; same fixture with `documentLocale: null` → `'€810.00'` (en). Existing `'$810.00'` / `'$0.00'` assertions (`:138-141`) stay (USD fixture, null locale → en).
- [ ] **Step 2: Run** `pnpm --filter @breeze/api test contractTemplateRender` → FAIL.
- [ ] **Step 3: Implement** as specified.
- [ ] **Step 4: Run** → PASS.
- [ ] **Step 5: Commit** — `refactor(api): contract template totals via shared formatMoney with the quote's document locale (#3777)`.
...
- Modify (comments only, behaviour unchanged — verify no locale copy): `apps/api/src/services/quoteService.ts:499-508` (`cloneQuote`), `apps/api/src/services/quoteAcceptService.ts:172-181` (quote→invoice), `:318-326` (quote→contract — contracts have no locale column)
...
- [ ] **Step 4: Run** → PASS; also `pnpm --filter @breeze/api test invoiceCheckout routes/portal/invoices routes/stripeConnect` (the widened `getPartnerStripeClient` return is additive; these must stay green untouched).
...
**Files:**
- Modify: `apps/api/src/routes/stripeConnect/index.ts:51-56` (POST /key response), `:68-83` (GET), new `POST /refresh` handler after GET
- Test: `apps/api/src/routes/stripeConnect/index.test.ts` (extend; mocks at `:81-83` and `:121`)
...
- Ticketing `hourlyRate` description → `'Override hourly rate in the ticket organization\'s currency (log_time_entry; defaults from ticket category / org rate)'`. `log_time_entry` output (`:684`): `JSON.stringify({ timeEntry: entry, currencyCode })` where `currencyCode` is read with one query `db.select({ currencyCode: organizations.currencyCode }).from(organizations).where(eq(organizations.id, entry.orgId)).limit(1)` (`organizations` from `'../db/schema'`; `entry.orgId` is on the returned time-entry row) — interim until wave 4 stamps `time_entries.currency_code`, which will replace this read with the row's own column.
...
- Modify (new `{{zero}}` token in all 8): `apps/web/src/locales/*/billing.json:890` `quotes.actions.sendConfirm.zeroTotalWarning`
