...
**Goal:** Every ticketing monetary row (time entry, ticket part) snapshots its currency at creation; org and category rate defaults carry the currency they were entered under; the rate chain is match-or-skip (a wrong-currency default never lands on an entry); invoice assembly partitions billables into `{included, blockedByCurrency}` against the draft header currency and offers an explicit old-currency draft so nothing is stranded; `issueInvoice` asserts time/part source currency under lock; ticket org-moves are blocked when the destination currency differs and unbilled monetary rows exist; billables CSV, ticket billing summary, timesheet money and `listBillables` report per-currency groups (B5, spec §7).

**Architecture:** Four new columns (`time_entries.currency_code`, `ticket_parts.currency_code`, `org_ticket_settings.rate_currency`, `ticket_categories.rate_currency`) in one idempotent migration with audited backfills and NOT VALID FKs to `supported_currencies`. Stamping is server-side only — no validator admits a client-supplied currency on entries/parts/settings/categories. `resolveTicketLink` becomes the single source of an entry's currency (it reads the ticket org's `currency_code` and returns the matching default rate with that currency). Assembly is advisory and non-locking (partition only); `issueInvoice` is authoritative (source currency re-checked on the rows it already locks — no new locks, no reordering). The move-org guard locks `time_entries → ticket_parts` (`ORDER BY id FOR UPDATE`) in the same relative order `issueInvoice` uses, so an in-flight issue and an in-flight move serialize instead of deadlocking.
...
- **Snapshots rule.** `time_entries.currency_code` / `ticket_parts.currency_code` are written exactly once (at creation, or at first attach for a standalone entry). Editing `hourly_rate` / `unit_price` never restamps; org currency changes never touch existing rows; the ticket-move rewrite touches `org_id` only. `org_ticket_settings.rate_currency` is restamped from the org only when `default_hourly_rate` is itself being (re)entered; `ticket_categories.rate_currency` only when `default_hourly_rate` is (re)entered.
...
- **Client never supplies currency** for entries/parts/org ticket settings/categories. Validators are unchanged except `assembleFromOrgSchema.currencyCode` and a new `assembleFromTicketQuerySchema` (the explicit "assemble a draft in <old currency>" path).
- **Error codes (new):** `TimeEntryServiceError` `CURRENCY_MISMATCH` (409 — relinking a stamped entry to an org in another currency); `TicketServiceError` `TICKET_MOVE_CURRENCY_BLOCKED` (409, carries `details`); `InvoiceServiceError` `ALL_BLOCKED_BY_CURRENCY` (409, carries `details.blockedByCurrency`); existing invoice `CURRENCY_MISMATCH` (400) reused for time/part source-vs-header at issue.
...
- **Lock ordering:** `issueInvoice` stays `invoices → invoice_lines → contracts → contract_lines → time_entries → ticket_parts` (wave 2); wave 4 adds a column to two already-locked selects and nothing else. `moveTicketOrg` guard: `time_entries → ticket_parts` (then the `tickets` UPDATE) — never touches `invoices`.
...
- Device org-move (`apps/api/src/routes/devices/moveOrg.ts:200-205`) also rewrites `time_entries`/`ticket_parts.org_id` for tickets bound to the moved device. **Deliberately left unguarded** (spec names only ticket moves): rows keep their snapshot and remain invoiceable via the old-currency draft path. Task 14 adds a code comment; flagged to the owner in Self-Review.
...
- DB constraints (SQL-only, per repo precedent at `tickets.ts:20-21`): `time_entries_currency_required_when_org_chk CHECK (org_id IS NULL OR currency_code IS NOT NULL)`; `ticket_categories_rate_currency_chk CHECK (default_hourly_rate IS NULL OR rate_currency IS NOT NULL)`; FKs `time_entries_currency_code_fkey`, `ticket_parts_currency_code_fkey`, `org_ticket_settings_rate_currency_fkey`, `ticket_categories_rate_currency_fkey` → `supported_currencies(code)`, added `NOT VALID` then validated with `foreign_key_violation` downgraded to a WARNING (exact precedent: `apps/api/migrations/2026-08-27-b-org-currency-and-fks.sql`).
...
--  * time_entries.currency_code      — nullable ONLY while org_id IS NULL (standalone
--                                      timesheet entries); stamped on org/ticket attach.
...
-- (categories) and are REPORTED via RAISE WARNING counts (repo convention).
...
```ts
  // Wave 4 (#3776): snapshot of the org currency at creation / first attach.
  // Nullable only while org_id IS NULL — CHECK time_entries_currency_required_when_org_chk
  // lives in SQL (2026-08-30-ticketing-currency.sql). Never restamped.
...
**Files:**
- Modify (add `currencyCode` to `timeEntries` inserts with `orgId` set, `currencyCode` to every `ticketParts` insert, `rateCurrency` to every `orgTicketSettings` insert): `apps/api/src/__tests__/integration/time-entries-rls.integration.test.ts:246,270,363,562` (entries) `:291,573` (parts); `ticket-move-org.integration.test.ts:104-126` (entry + part); `ticket-config-rls.integration.test.ts:261,282,493,572,630` (org settings); `invoiceIssueRace.integration.test.ts:113`; `changeCurrency.integration.test.ts:126`; `invoiceCurrencyStamping.integration.test.ts:46`; `apps/api/src/services/invoiceService.issue.integration.test.ts:60`.

**Interfaces:**
- Every fixture stamps the currency of the org it inserts under (read it from the seeded org's `currencyCode`, or hard-code the same literal the org was seeded with — `'USD'` for the legacy suites, `'EUR'` where the org is EUR). `ticketCategories` inserts (`ticket-validation-rls:81,92`, `time-entries-rls:93`) set no rate → untouched.

- [ ] **Step 1: Re-grep before editing** — `git grep -n "insert(timeEntries)\|insert(ticketParts)\|insert(orgTicketSettings)\|INSERT INTO \(time_entries\|ticket_parts\|org_ticket_settings\)" -- 'apps/' 'ee/'` — every hit with an org gets a stamp (line numbers above are pre-change references).
...
- [ ] **Step 3: Run** — `pnpm --filter @breeze/api test:integration time-entries-rls ticket-move-org ticket-config-rls invoiceIssueRace changeCurrency invoiceCurrencyStamping invoiceService.issue` → PASS (no 23502/23514).
...
- `upsertOrgTicketSettings(orgId: string, input: OrgTicketSettingsInput, orgCurrencyCode: string)` — new third parameter. Insert values: `{ orgId, rateCurrency: orgCurrencyCode, ...fields }`. Conflict `set`: `{ ...fields, ...(input.defaultHourlyRate !== undefined ? { rateCurrency: orgCurrencyCode } : {}), updatedAt: new Date() }` — the currency is restamped **only** when the rate itself is being (re)entered; a `slaOverrides`-only or `defaultBillable`-only PATCH leaves the stored rate/currency pair intact.
...
- [ ] **Step 1: Write failing unit tests.** `ticketConfigService.test.ts`: (a) `upsertOrgTicketSettings(ORG, { defaultHourlyRate: 125.5 }, 'EUR')` → `insertedValues[0].rateCurrency === 'EUR'` and `conflictArgs[0].set.rateCurrency === 'EUR'`; (b) `upsertOrgTicketSettings(ORG, { defaultBillable: true }, 'EUR')` → insert values carry `rateCurrency: 'EUR'` but `conflictArgs[0].set` has **no** `rateCurrency` key; (c) `getOrgTicketSettings` default case (`:489`) now `toEqual({ orgId, slaOverrides: {}, defaultHourlyRate: null, defaultBillable: null, rateCurrency: null })`; the `:509` expectation gains `rateCurrency` from the mocked returning row (add `rateCurrency: 'USD'` to `dbMocks.insertResult`). `orgTicketSettings.test.ts`: extend the `'../db/schema'` mock (`:54-56`) to `organizations: { id: 'id', deletedAt: 'deletedAt', currencyCode: 'currencyCode' }`, make `dbSelectResult` resolve `[{ id: ORG_ID, currencyCode: 'CAD' }]`, and assert `serviceMocks.upsertOrgTicketSettings` was called with `(ORG_ID, body, 'CAD')`. `validators/ticketConfig.test.ts`: `orgTicketSettingsSchema.parse({ defaultHourlyRate: 10, rateCurrency: 'EUR' })` has no `rateCurrency` key.
...
export async function upsertOrgTicketSettings(orgId: string, input: OrgTicketSettingsInput, orgCurrencyCode: string) {
  ...
  // (spec §7). Stamped on insert; restamped on conflict ONLY when the rate is
  // being (re)entered — never because an unrelated field changed.
  const restamp = input.defaultHourlyRate !== undefined ? { rateCurrency: orgCurrencyCode } : {};
  const [row] = await db
...
```
`toOrgTicketSettingsResponse` row type gains `rateCurrency?: string | null` and the response gains `rateCurrency: row?.rateCurrency ?? null`; `getOrgTicketSettings` and `getOrgBillingDefaults` selects gain `rateCurrency: orgTicketSettings.rateCurrency`. Route: `resolveAccessibleOrg` selects `{ id: organizations.id, currencyCode: organizations.currencyCode }`, returns `{ id, currencyCode: orgRows[0].currencyCode }`; PATCH passes it through.
...
- PATCH: `typeof body.defaultHourlyRate === 'number'` → resolve the owning partner (partner scope: `auth.partnerId`; system scope: `select partnerId from ticketCategories where id` → 404 if missing) and set `rateCurrency = await partnerCurrency(pid)`; `body.defaultHourlyRate === null` → `rateCurrency: null`; undefined → key omitted (untouched). The DB CHECK `ticket_categories_rate_currency_chk` backstops a rate without a currency.
...
```ts
    if (typeof body.defaultHourlyRate === 'number') {
      let pid: string | null = auth.scope === 'partner' ? (auth.partnerId ?? null) : null;
      if (!pid) {
        const catRows = await db.select({ partnerId: ticketCategories.partnerId })
          .from(ticketCategories).where(eq(ticketCategories.id, id)).limit(1);
        pid = catRows[0]?.partnerId ?? null;
        if (!pid) return c.json({ error: 'Category not found' }, 404);
      }
      const cur = await partnerCurrency(pid);
      if (!cur) return c.json({ error: 'Partner not found' }, 404);
      set.rateCurrency = cur;
    } else if (body.defaultHourlyRate === null) {
...
### Task 7: Stamp currency on entries and parts; snapshots never restamp

**Executor:** codex (depends on Task 6 — `resolveTicketLink(...)` now returns `currencyCode: string`).

**Files:**
- Modify: `apps/api/src/services/timeEntryService.ts:214-252` (`createTimeEntry`), `:297-354` (`startTimer` insert), `:449-481` (`updateTimeEntry` relink), `:620-645` (`addTicketPart`), `:654-669` (`updateTicketPart` — comment), `:701-724` (`entrySelection`)
...
**Interfaces:**
- `createTimeEntry` / `startTimer`: insert `currencyCode: orgId ? link.currencyCode : null` (standalone entries stay `null`; the DB CHECK enforces the org⇒currency rule). An explicit `input.hourlyRate` is interpreted in `link.currencyCode`.
- `updateTimeEntry` relink (`input.ticketId` string): after the existing partner check, if `entry.currencyCode == null` → `set.currencyCode = link.currencyCode`; else if `entry.currencyCode !== link.currencyCode` → `throw new TimeEntryServiceError(\`This entry is in ${entry.currencyCode}; the ticket's organization bills in ${link.currencyCode} — snapshots are never restamped\`, 409, 'CURRENCY_MISMATCH')`. Detach (`input.ticketId === null`) leaves `currencyCode` untouched. `input.hourlyRate` edits never touch `currencyCode` (`getEntryOr404` selects the full row, so `entry.currencyCode` is available).
- `addTicketPart`: insert `currencyCode: link.currencyCode`.
- `updateTicketPart`: `set` must never contain `currencyCode` — add the snapshot comment above the function.
...
- [ ] **Step 1: Write failing tests** (mock conventions: `dbMocks.selectResults` queue in order ticket → org → category; `dbMocks.insertedValues[0]` for inserts; `dbMocks.updateSetArgs[0]` for update `set`): (a) ticket-linked create with org `currencyCode: 'EUR'` → `insertedValues[0].currencyCode === 'EUR'`; (b) standalone create → `currencyCode === null`; (c) `startTimer` with a ticket → `'EUR'`; (d) `updateTimeEntry` rate-only edit on an entry `{ currencyCode: 'EUR' }` → `updateSetArgs[0]` has **no** `currencyCode` key; (e) relink of a standalone entry (`currencyCode: null`) to a USD-org ticket → `set.currencyCode === 'USD'`; (f) relink of a `'EUR'` entry to a USD-org ticket → rejects `{ code: 'CURRENCY_MISMATCH', status: 409 }`; (g) detach → `set` has no `currencyCode`; (h) `addTicketPart` → `insertedValues[0].currencyCode === link currency`; (i) `updateTicketPart({ unitPrice: 5 })` → `set` has no `currencyCode`.
...
```ts
  ...
  // ... insert values gain:
      // Snapshot (spec §7): null only for standalone entries; never restamped.
      currencyCode,
```
`updateTimeEntry` relink branch:
```ts
...
- [ ] **Step 5: Commit** — `feat(api): time entries and parts snapshot their currency at creation (#3776)`.
...
- `getTimesheet(...)` → existing shape plus `totals.billableAmounts: CurrencyAmount[]` computed in JS from the entries already fetched (`isBillable && hourlyRate != null && currencyCode != null`; `(durationMinutes/60) * hourlyRate`, grouped by `currencyCode`, first-seen order).

- [ ] **Step 1: Write failing tests.** `timeEntryService.test.ts`: `getTicketBillingSummary` with queue `[{ totalMinutes: 90, billableMinutes: 60 }]`, `[{ currencyCode: 'EUR', amount: '100.00' }, { currencyCode: 'USD', amount: '25.00' }]`, `[{ partsCount: 2 }]`, `[{ currencyCode: 'EUR', amount: '40.00' }]` → exact object; empty queues → `billableAmounts: []`, `billableTotals: []`, zeros. `getTimesheet`: two EUR entries (60 min @ 100, 30 min @ 100) + one USD entry (60 min @ 50) + one non-billable → `totals.billableAmounts` `[{ currencyCode: 'EUR', amount: '150.00' }, { currencyCode: 'USD', amount: '50.00' }]`. `parts.test.ts:271-277`: update the mocked summary to the new shape and assert `body.data.time.billableAmounts[0].amount === '125.00'`.
- [ ] **Step 2: Run to verify failure.**
- [ ] **Step 3: Implement:**
...
export async function getTicketBillingSummary(ticketId: string) {
  ...
  const timeMoney = await db
    .select({
      ...
      amount: sql<string>`COALESCE(SUM((${timeEntries.durationMinutes}::numeric / 60) * ${timeEntries.hourlyRate}), 0)::numeric(12,2)`
    })
    .from(timeEntries)
    .where(and(
...
```ts
  ...
  for (const entry of entries) {
    ...
    const amount = ((entry.durationMinutes ?? 0) / 60) * Number(entry.hourlyRate);
...
- [ ] **Step 1: Write failing tests.** `timeEntryService.test.ts`: queue one time row (`minutes: 30, rate: '100.00', currencyCode: 'EUR'`) and one part row (`quantity: '2.00', unitPrice: '10.00', currencyCode: 'USD'`) → `rows[0].currencyCode === 'EUR'`, `rows[1].currencyCode === 'USD'`, `totalsByCurrency` `[{ currencyCode: 'EUR', amount: '50.00' }, { currencyCode: 'USD', amount: '20.00' }]`. `parts.test.ts:281-300`: `listBillables` mock resolves `{ rows: [...existing row with currencyCode: 'USD'], totalsByCurrency: [] }`; header assertion becomes `'type,date,organization,ticket,description,technician,quantity,rate,amount,currency,billing_status,approved'`; assert the data line contains `,62.50,USD,not_billed,`.
- [ ] **Step 2: Run to verify failure.**
...
### Task 13: Ticket org-move blocked on currency mismatch with unbilled monetary rows

**Executor:** claude (lock order inside an existing transaction).

**Files:**
- Modify: `apps/api/src/services/ticketService.ts:33-46` (error code), `:47-56` (`TicketServiceError` gains `details`), `:1337-1392` (`moveTicketOrg`)
- Modify: `apps/api/src/routes/tickets/moveOrg.ts:38-42`, `apps/api/src/services/aiToolsTicketing.ts:47-52` (`serviceErrorToJson` includes `details`)
- Test: `apps/api/src/services/ticketService.test.ts:2503-2610`, `apps/api/src/routes/tickets/moveOrg.test.ts`, `apps/api/src/__tests__/integration/ticket-move-org.integration.test.ts` (+ blocked case)

**Interfaces:**
- `TicketServiceErrorCode` gains `'TICKET_MOVE_CURRENCY_BLOCKED'`; `TicketServiceError` constructor gains `public details?: Record<string, unknown>` (4th param).
- `moveTicketOrg`: the org select adds `currencyCode: organizations.currencyCode`. Inside `db.transaction`, **before** the `tickets` UPDATE, when `sourceOrg.currencyCode !== targetOrg.currencyCode`:
```ts
      // Lock order matches issueInvoice (time_entries → ticket_parts, ORDER BY id
      // FOR UPDATE) so a concurrent issue serializes on the same rows instead of
      // deadlocking. A move that wins the race rewrites org_id, which issueInvoice's
      // locked org check then rejects (SOURCE_NOT_FOUND) — never a cross-org line.
      const lockedTime = await tx.select({ id: timeEntries.id }).from(timeEntries)
        .where(and(eq(timeEntries.ticketId, ticketId), eq(timeEntries.billingStatus, 'not_billed'),
          eq(timeEntries.isBillable, true), isNotNull(timeEntries.hourlyRate)))
        .orderBy(timeEntries.id).for('update');
      const lockedParts = await tx.select({ id: ticketParts.id }).from(ticketParts)
        .where(and(eq(ticketParts.ticketId, ticketId), eq(ticketParts.billingStatus, 'not_billed'),
          eq(ticketParts.isBillable, true)))
        .orderBy(ticketParts.id).for('update');
      if (lockedTime.length || lockedParts.length) {
        throw new TicketServiceError(
          `Cannot move: ${lockedTime.length} unbilled time entries and ${lockedParts.length} unbilled parts are in ${sourceOrg.currencyCode} but ${targetOrg.name} bills in ${targetOrg.currencyCode}. Invoice them first (or assemble a ${sourceOrg.currencyCode} draft), then move.`,
          409, 'TICKET_MOVE_CURRENCY_BLOCKED',
          { sourceCurrency: sourceOrg.currencyCode, targetCurrency: targetOrg.currencyCode,
            unbilledTimeEntries: lockedTime.length, unbilledParts: lockedParts.length }
        );
      }
```
The `TICKET_ORG_DENORMALIZED_TABLES` rewrite stays `SET org_id = ...` only — `currency_code` is never touched (snapshot). Same-currency moves are unchanged (no locks taken).
- Route `moveOrg.ts`: `catch (err) { if (err instanceof TicketServiceError && err.code === 'TICKET_MOVE_CURRENCY_BLOCKED') return c.json({ error: err.message, code: err.code, details: err.details }, 409); return handleServiceError(c, err); }`. `serviceErrorToJson` returns `{ error, code, details }`.

- [ ] **Step 1: Write failing unit tests** in `ticketService.test.ts` (the org lookup stub at `:2518-2521` gains `currencyCode`; the test-file `db.transaction` mock must expose `tx.select(...).from(...).where(...).orderBy(...).for(...)` resolving from a queue — extend `dbMocks` accordingly): (a) differing currency + one locked time row → rejects `{ code: 'TICKET_MOVE_CURRENCY_BLOCKED', status: 409, details: { unbilledTimeEntries: 1, unbilledParts: 0 } }` and `txExecuteMock` never ran; (b) differing currency, both lock selects empty → rewrite proceeds (3 `txExecuteMock` calls); (c) same currency → no `tx.select` calls at all. `moveOrg.test.ts`: 409 body carries `code` + `details`.
- [ ] **Step 2: Run to verify failure.**
- [ ] **Step 3: Implement** as above (`isNotNull` from `drizzle-orm`).
- [ ] **Step 4: Integration** — extend `ticket-move-org.integration.test.ts`: seed orgA `USD`, orgB `EUR` (same partner); with the Task-3-stamped `not_billed` billable entry (`hourlyRate: '100.00'`) → `moveTicketOrg` rejects 409 `TICKET_MOVE_CURRENCY_BLOCKED` and `tickets.org_id` is unchanged; mark the entry `billed` → the move succeeds and `time_entries.currency_code` is still `'USD'` while `org_id` is now orgB (snapshot permanence).
- [ ] **Step 5: Run** `pnpm --filter @breeze/api test ticketService routes/tickets/moveOrg aiToolsTicketing` + `test:integration ticket-move-org` → PASS.
- [ ] **Step 6: Commit** — `feat(api): block ticket org-moves across currencies while unbilled monetary rows exist (#3776)`.

### Task 14: AI tool copy, validator comments, device-move note
...
- Modify: `apps/api/src/routes/devices/moveOrg.ts:197-205` (comment only)
...
- `routes/devices/moveOrg.ts` above the `time_entries` UPDATE: `// Wave 4 (#3776): org_id only — currency_code is a snapshot and is NOT rewritten. A device move across currencies is deliberately unguarded (spec §7 guards ticket moves only); affected rows stay invoiceable through the old-currency draft path (assembleDraftFromOrg currencyCode override).`
...
- Modify: `apps/web/src/components/settings/TicketCategoriesPage.tsx:13-24` (type), `:85-92` (`describe`), page load (fetch partner currency once)
...
**Interfaces:**
- `OrgTicketSettings` gains `rateCurrency: string | null`. `load()` additionally fetches `GET /orgs/organizations/${orgId}` (returns the org row top-level; read `currencyCode`) and `GET /orgs/partners/me` (read `currencyCode`); store `orgCurrency` / `partnerCurrency` state. When both loaded and `orgCurrency !== partnerCurrency`, render `<p data-testid="org-ticket-currency-nudge" className="mt-2 rounded-md border border-amber-300 bg-amber-50 p-2 text-xs text-amber-900">{t('orgTicketSettingsEditor.billing.currencyNudge', { orgCurrency, partnerCurrency })}</p>` above the rate input. The rate label renders `t('orgTicketSettingsEditor.billing.hourlyRate', { currency: orgCurrency ?? '' })`.
- `Category` gains `rateCurrency: string | null`; the page fetches `/orgs/partners/me` once and passes `partnerCurrency` into `describe(c, partnerCurrency)`; rate formats with `formatCurrency(parseFloat(c.defaultHourlyRate), c.rateCurrency ?? partnerCurrency)`; the form label uses `t('ticketCategoriesPage.defaultHourlyRate', { currency: partnerCurrency })`.

- [ ] **Step 1: Write failing tests.** `OrgTicketSettingsEditor.test.tsx`: extend `mockApi` with `/orgs/organizations/${ORG_ID}` → `{ currencyCode: 'EUR' }` and `/orgs/partners/me` → `{ currencyCode: 'USD' }`; assert the nudge renders; with both `'USD'` it does not. `TicketCategoriesPage.test.tsx`: a category `{ defaultHourlyRate: '100.00', rateCurrency: 'CAD' }` renders a CAD-formatted rate (`/CA\$|CAD/`).
...
- `TsEntry` gains `currencyCode: string | null`; `TsSheet.totals` gains `billableAmounts`; the week header renders one `formatMoney(amount, currencyCode)` chip per entry under `data-testid="timesheet-billable-amounts"`; the entry rate cell shows `formatMoney(rate, entry.currencyCode ?? 'USD')` — a null-currency standalone entry keeps the USD fallback only for display (no money is computed from it server-side).

- [ ] **Step 1: Update/extend tests**: `TicketTimeBilling.test.tsx` fixture → new shape with `billableAmounts: [{ currencyCode: 'EUR', amount: '150.00' }]`; assert `€150.00` (or the `Intl` output for the test locale). `TicketPartsCard.test.tsx` fixture gains `currencyCode: 'EUR'`; assert `€`. `TimesheetPage.test.tsx`: `totals.billableAmounts` renders two chips for `[EUR, USD]`.
...
### Task 17: Web — assembly blocked groups + old-currency draft; move-org block guidance

**Executor:** claude (UI).

**Files:**
- Modify: `apps/web/src/components/billing/InvoicesPage.tsx:119-127` (state), `:223-256` (`submitDialog`), `:742-805` (dialog fields)
- Modify: `apps/web/src/components/tickets/TicketWorkbench.tsx:397-415` (`createInvoice`), `:417-423` (`handleMoveOrg`), `:830-850` (move form)
...
- `TicketWorkbench.createInvoice`: on `ActionError` `ALL_BLOCKED_BY_CURRENCY` set `invoiceBlocked` state from `err.body.details.blockedByCurrency` and render a small panel under the invoice button with one `<button data-testid="ticket-assemble-in-{code}">` per group that POSTs `/tickets/${ticketId}/invoice?currencyCode=${code}` and navigates on success. `handleMoveOrg`: on `ActionError` `TICKET_MOVE_CURRENCY_BLOCKED` set `moveBlocked` from `err.body.details` and render `<p data-testid="ticket-move-blocked-currency">{t('ticketWorkbench.move.blockedCurrency', { timeCount, partCount, sourceCurrency, targetCurrency, targetOrg })}</p>` inside the move form (the toast from `runAction` still fires with the server message).

- [ ] **Step 1: Write failing tests**: `InvoicesPage.test.tsx`: mock the assemble POST to return 409 `{ error, code: 'ALL_BLOCKED_BY_CURRENCY', details: { blockedByCurrency: [{ currencyCode: 'EUR', count: 2, amount: '250.00' }] } }` → blocked panel renders with `invoices-assemble-in-EUR`; clicking it re-POSTs with `currencyCode: 'EUR'`. `TicketWorkbench.test.tsx`: move-org POST returning 409 `TICKET_MOVE_CURRENCY_BLOCKED` with details → `ticket-move-blocked-currency` renders; invoice POST returning 409 `ALL_BLOCKED_BY_CURRENCY` → `ticket-assemble-in-EUR` renders and re-POSTs with `?currencyCode=EUR`.
...
**Files:**
- Modify (all 8 locales: `en`, `de-DE`, `es-419`, `fr-CA`, `fr-FR`, `it-IT`, `pt-BR`, `tr-TR` under `apps/web/src/locales/<locale>/`): `settings.json` keys `ticketCategoriesPage.defaultHourlyRate` (`en:2254`) and `orgTicketSettingsEditor.billing.hourlyRate` (`en:2755`) → `"Default hourly rate ({{currency}})"`; new `orgTicketSettingsEditor.billing.currencyNudge`; `tickets.json` new `ticketWorkbench.invoice.blockedByCurrency`, `ticketWorkbench.invoice.assembleIn`, `ticketWorkbench.move.blockedCurrency`, `ticketTimeBilling.noAmount`; `billing.json` new `invoicesPage.dialog.currency`, `invoicesPage.dialog.currencyOrgDefault`, `invoicesPage.dialog.blockedByCurrency`, `invoicesPage.dialog.allBlockedByCurrency`, `invoicesPage.dialog.assembleIn`.
- Modify: `apps/docs/src/content/docs/features/ticketing.mdx:252` (per-org rate is in the org's currency; partner category rates apply only when currencies match), `:282-284` (CSV gains a `currency` column; totals are per currency), plus one sentence under the move-org section (blocked across currencies while unbilled work exists).
...
- [ ] **Step 3: Update the docs** with the `update-breeze-docs` skill conventions.
...
**Interfaces:**
- Backfill test: open a dedicated `postgres(DATABASE_URL, { max: 1, onnotice: (n) => notices.push(n.message) })` client (pattern: `backupProfilesPartnerRls.integration.test.ts:732`); inside ONE transaction: `ALTER TABLE ticket_parts ALTER COLUMN currency_code DROP NOT NULL`, `ALTER TABLE org_ticket_settings ALTER COLUMN rate_currency DROP NOT NULL`, `ALTER TABLE time_entries DROP CONSTRAINT time_entries_currency_required_when_org_chk`, `ALTER TABLE ticket_categories DROP CONSTRAINT ticket_categories_rate_currency_chk`; insert 2 org-linked entries, 1 part, 1 org settings row, 1 category with a rate — all with NULL currency; replay `readFileSync(path.join(__dirname, '../../../migrations/2026-08-30-ticketing-currency.sql'))` via `client.unsafe(sql)`; assert `notices` contains `backfilled currency_code on 2 time_entries`, `... 1 ticket_parts`, `... 1 org_ticket_settings`, `... 1 ticket_categories`, and the rows now carry the org/partner currency; `ROLLBACK` (DDL is transactional — the live schema is untouched; `autoMigrate.test.ts` keeps the path reference honest).
- Snapshot permanence (real DB, service-level): (a) `updateTimeEntry(id, { hourlyRate: 200 })` leaves `currency_code` unchanged; (b) flipping `organizations.currency_code` leaves every existing `time_entries`/`ticket_parts`/`org_ticket_settings` row unchanged; (c) `upsertOrgTicketSettings(org, { defaultBillable: false }, 'GBP')` after the flip leaves `rate_currency` at the old value, while `{ defaultHourlyRate: 90 }` restamps to `'GBP'`.
...
- [ ] **Step 2:** Re-grep the stamping blind spots: `git grep -n "insert(timeEntries)\|insert(ticketParts)\|insert(orgTicketSettings)\|insert(ticketCategories)" -- 'apps/' 'ee/'` — every production insert stamps currency (or has no rate, for categories); `git grep -n "billableAmount\b\|billableTotal\b" -- 'apps/'` → zero hits (old scalar shape gone everywhere, including mocks).
...
- Deliberate scope calls, flagged for the owner: (a) relinking an already-stamped entry to an org in another currency **rejects** (`CURRENCY_MISMATCH` 409) rather than allow-and-strand — the snapshot rule has no "restamp on relink" exception; (b) `getTimesheet` had no money at all — "timesheet money per-currency" is implemented as `totals.billableAmounts` computed from the fetched entries (no per-day money); (c) the device org-move (`routes/devices/moveOrg.ts:200-205`) is left unguarded with a comment — affected rows stay invoiceable via the old-currency draft; (d) `org_ticket_settings.rate_currency` restamps only when `defaultHourlyRate` is present in the PATCH; (e) the ticket-path currency override travels as a query param (`?currencyCode=`) because the endpoint is body-less and an optional JSON validator rejects an empty body — CLAUDE.md's hash-vs-query rule governs web UI state, not API inputs; (f) `getTicketBillingSummary` drops the scalar `billableAmount`/`billableTotal` fields outright (a cross-currency sum is the bug class this program kills) — web consumers are updated in the same PR.
