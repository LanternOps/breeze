...
## Quorum amendments (2026-09-22, Opus 5.5 advisor pass) — THESE OVERRIDE THE TASK TEXT BELOW

Quorum: Opus 5.5 orchestrator + independent Opus 5.5 reviewer against `origin/main` `33b29159e6` (standing in for the owed Codex pass). Verdicts: D10/D11/D12/D13 **confirm with changes**, D14/D15/D16 **confirm**. Line numbers in the tasks have drifted +28…+55; re-grep.

- **Q1 (D11, keyset precision — blocking).** All four keyset sort columns are `timestamp` *without* time zone (`alerts.triggered_at`, `agent_logs.created_at`, `audit_logs.timestamp`, `device_change_log.timestamp`) and hold microseconds. Never round-trip the sort key through JS `Date`/`toISOString()` (ms) and never cast to `timestamptz`. Select the key as `col::text`, carry that text in the cursor, compare with `(col, id) < ($t::timestamp, $i::uuid)`. Test: two rows in the same millisecond, different microseconds, across a page boundary — neither skipped nor duplicated.
- **Q2 (D11, cursor input hygiene).** `readKeysetArgs` validates `t` (timestamp text regex) and `i` (UUID regex) before any SQL and returns the typed `CURSOR_MISMATCH`/`INVALID_CURSOR` tool error otherwise — a malformed value must never reach a `::uuid`/`::timestamp` cast (22P02 poisons the request transaction). Opt-in projection flags (`include*`) are excluded from the fingerprint. `offset` is capped (max 10 000).
- **Q3 (D12/D13, windows vs compactor — blocking).** Any character window a tool emits (`read_artifact.text`, `get_script_execution` stdout/stderr) must survive `compactToolResultForChat` intact, else `nextOffset` skips text the model never saw. Either size windows by *JSON-escaped* length to fit the lowest compaction tier that can apply, or have the compactor recognise the window shape structurally (as `asCaptureEnvelope` does) and pass it through. `stderr` default ≤ the sanitizer cap (1 200). Fixtures must be escape-heavy (newlines, quotes, backslashes) and pushed through `compactToolResultForChat`, not only measured raw.
- **Q4 (D13a, read scope — blocking).** `read_artifact` reads with the SAME anchor capture wrote with: thread the capture context `executeTool` already computes (`aiTools.ts:~655`, `captureContextFrom`) through `ToolExecutionContext`; match `run_id` exactly or the *current* `session_id` exactly (not "any session of this user"); require `expires_at > now()`. Refuse otherwise with a typed error.
- **Q5 (D13b, legacy raw artifacts).** Artifacts written before redact-then-capture are raw. `read_artifact` excludes `kind='input_capture'` rows with `created_at` before a `REDACTED_CAPTURE_SINCE` constant set in the PR (deploy-time cutoff) and excludes `export_dataset` artifacts entirely this wave. Also: head/tail previews are built from the REDACTED body. Land redact-then-capture as its own commit first.
- **Q6 (D13c — scope cut).** Drop Task 7's `resources/read breeze://artifacts/{id}`: MCP `auth.user.id` is the key creator, so it would serve the creator's chat/Helper artifacts to an OAuth client or service key, and MCP never captures anyway. Task 7 is reduced to `supportsResourceLinks` + tests, or deferred entirely to Feature B together with MCP anchoring (file the follow-up in Task 8). Default: **defer Task 7**.
- **Q7 (D12, fourth schema surface — blocking).** `scriptBuilderTools.ts` declares its own Zod shapes for `query_devices`, `manage_alerts`, `get_script_execution`. Add the paging/window params there too, and include `buildScriptBuilderTools` in Task 4's parity contract; otherwise the resolver tells that model to page with a param Zod strips → page 1 again. `pageZodShape`/`keysetZodShape` carry `.describe()` text (SDK param descriptions are not derived from the registry).
- **Q8 (D12, resolver robustness).** The hint resolver is wrapped in try/catch → `'none'`. `nextStep` fires only on array-item truncation, not on `system_cleanup`'s output-tail cuts (it reuses `arrayItemsDropped`, `aiToolOutput.ts:~547-578`) — give that branch its own counter or exclude it.
- **Q9 (Task 4 lint — blocking).** Widen `LIMIT_TEXT`'s noun list (`Max devices (default 50, max 200)` must pass). ~30 of 76 existing `limit` descriptions fail today: the reword is ONE serial step owned by one agent, done before Tasks 5a–5c start.
- **Q10 (D10).** `hotForShaping` ranks `chat` + `helper` rows only; size columns computed with `octet_length` are named `*Bytes`, not `*Chars` (or use `char_length`). `manage_tickets`, `list_monitors`, `get_incident_timeline` are MCP-only (not in `TOOL_TIERS`) — D10 cannot measure them; say so in the baseline doc.
- **Q11 (facts).** `manage_patches` schema is in `aiToolSchemasFleet.ts`; `list_ai_agents` goes through the `listAgents` service (page inside the service, not a `.limit` chain); fix the `query_devices` test mock so COUNT returns a count row; `export_dataset` is absent from web `tierConfig.ts` (don't copy that gap for `read_artifact`).

**Execution order (file-conflict map):** Round 1 parallel — Task 1; Task 2; Task 6 service layer (`blobStorage.openRange`, `artifactService`, `toolResultCapture` redact-then-capture). Then serial: Task 3 → Task 4 (incl. Q9 reword) → 5a → 5b → 5c → Task 6 registration → Task 8. One owner at a time for `aiToolOutput.ts`, `aiToolSchemas.ts`, `aiAgentSdkTools.ts`.
