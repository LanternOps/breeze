...
- **UTC everywhere.** Never `date_trunc('day', <timestamptz>)` — it depends on the session timezone. Bucket with `(<column> AT TIME ZONE 'UTC')::date` and bound half-open `>= (<from>::date) AT TIME ZONE 'UTC'` / `< (<to>::date + 1) AT TIME ZONE 'UTC'`. `through` is always the last COMPLETE UTC day (today−1); the current UTC day is never in a window.
...
docker compose -p breeze-test-ai-agents-p2-6 -f docker-compose.test.yml up -d postgres
...
cd apps/api && npx vitest run --config vitest.config.rls.ts src/__tests__/integration/rls-coverage.integration.test.ts
```
...
- [ ] **Contract suites.** `rls-coverage` (under `vitest.config.rls.ts`), `tenantCascade`, `tenant-export-policy`, `tenantExportErasureRoundtrip`, `orgMergeRegistry`, `orgMerge` (all under `vitest.integration.config.ts`); `workerEntrypointClosure.contract.test.ts`, `scheduleRegistry.contract.test.ts`, `composeBindMounts.test.ts`, `partner-wide-write-coverage.test.ts`, `autoMigrate.test.ts` (unit). Web: `localeParity`, `translationCoverage`, `no-silent-mutations`, `Sidebar.nav`, `routeScope`.
