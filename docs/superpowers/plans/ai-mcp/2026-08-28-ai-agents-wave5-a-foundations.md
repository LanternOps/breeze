...
- **PR-B pointers (documented, NOT touched):** `intentReleaseWorker.ts:295-317` (approved-human-request fetch), `revalidateRelease.ts:41-64` (digest chain), `agentReleaseAuthority` predicate, the outbox `intent_created` no-op branch (`intentReleaseWorker.ts` doc comment) — all byte-identical in this PR.
