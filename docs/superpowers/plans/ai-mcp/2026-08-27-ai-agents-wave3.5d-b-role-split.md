...
**Final placement (filled 2026-08-27, Task 5):** global = 76, socket-owner = 28 (out of 104 total). The closure tool confirmed every one of Task 1's initial-pass guesses except one: `policyEvaluationWorker` was mis-flagged `global` and is flipped to `socket-owner` here — its runtime closure reaches `routes/agentWs.ts` via a dynamic import one hop out (`jobs/policyEvaluationWorker.ts` → `services/policyEvaluationService.ts` → `await import('../jobs/automationWorker')` for `enqueueAutomationRun` → `services/automationRuntime.ts` → `services/scriptDispatch.ts` → `routes/agentWs.ts`). All other sanity-expected classifications held: monitor/snmp/backup/discovery/networkBaseline are ALL socket-owner (`services/agentCommandRelay.ts:25` value-imports `isAgentConnected`/`sendCommandToAgent` from `routes/agentWs.ts` — runtime-safe behind a `breezeRole() !== 'worker'` guard, but still a static edge the mechanical closure walk correctly counts); automation/quickSupport/softwareDeployment/commandQueue-reachers socket-owner. Consequence: this whole family of heavy agent-I/O workers stays on the `api`/`all` container this wave, not `worker` — migrating `agentCommandRelay.ts`'s `routes/agentWs` import to a lazy `await import(...)` inside its guarded branches (filed follow-up, see the "Do not relitigate" line above) is the one-line change that would let all five move to `global` in a later wave.
