...
**Goal:** Close findings MCP-OAUTH-01..12 per the approved design `docs/superpowers/specs/2026-07-11-mcp-oauth-security-hardening-design.md` (read the matching section before each task).
...
- Unit verification per task: `corepack pnpm@10.33.4 --filter @breeze/api test` (and `--filter @breeze/web test` for Task 8) + `--filter @breeze/api typecheck`. Migration tasks also run `corepack pnpm@10.33.4 db:check-drift`.
