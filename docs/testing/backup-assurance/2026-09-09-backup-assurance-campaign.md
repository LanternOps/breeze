...
| D6 | HIGH (no incremental) | I1 (Linux, macOS) | An unchanged second run re-uploaded all 10,047 files (10,046 new objects, `referencedFiles: 0`, every run-2 `backupPath` under the run-2 prefix). The helper logged no dedupe decision at all for run 2 (run 1 had logged "no previous snapshot for this destination"). Incremental backups — unconditional per `backup.go:729` — are not happening on the S3 path. | analysis delegated (see ledger) |
...
| D9 | HIGH (BMR unusable) | B1 (Linux) | Freshly minted, `active` recovery tokens are rejected by `POST /backup/bmr/recover/authenticate` (`Invalid recovery token`), both from the helper on the fresh VM and via direct curl. | analysis delegated |
