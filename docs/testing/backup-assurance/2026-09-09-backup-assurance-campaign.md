...
| S0 SQL discovery | WIN-A (SQL Server 2025 Express) | 0.112.7 | PASS after D20 — instance `SQLEXPRESS` 17.0.1000.7 returned (`databases: null`: the helper's `sqlcmd` enrichment does not find/trust the ODBC 18 `sqlcmd`, see O28); nothing persisted to `sql_instances` (D20b) | `phase2/mssql-instances2.json` |
| S1 SQL on-demand full backup of `AssureDB` | WIN-A | 0.112.7 | FAIL (D20b) — same `backup not configured on this device` | `phase2/cells-sql.txt` |
