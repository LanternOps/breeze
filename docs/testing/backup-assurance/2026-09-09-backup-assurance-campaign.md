...
| D20b | HIGH (every policy-managed device) | SQL S1 / Hyper-V H1 | With D20 fixed, the helper answers `backup not configured on this device`: when the device has no `agent.yaml` backup manager (the normal state for policy-managed devices) `main.go` only routes `bmr_recover`/`backup_verify`/`backup_test_restore`/`backup_stop` and fails everything else, and the on-demand MSSQL/Hyper-V payloads carry no destination provider config (unlike `backup_run`, which builds an ephemeral manager from its payload). Discovery still persisted nothing (`sql_instances` empty). | fix in flight (payload carries the provider config, helper builds the manager from it, discovery upsert); tracked under [#5447](https://github.com/LanternOps/breeze/issues/5447) |
| D22 | HIGH (every SQL Server 2022/2025 host) | SQL S1 | `mssql_backup` fails in `sqlcmd`: `SSL Provider: The certificate chain was issued by an authority that is not trusted` — the ODBC 18 `sqlcmd` shipped with SQL 2022+ defaults to mandatory encryption with certificate validation and the helper passes no `-C`; discovery enrichment fails the same way. Lookup fallback paths stop at `\\160\\`. | fix in flight; [#5457](https://github.com/LanternOps/breeze/issues/5457) |
...
| H1 Hyper-V on-demand backup of `WIN10-C` | KIT | 0.112.7 | FAIL (D20b) — `backup not configured on this device`: the helper has no `agent.yaml` manager and the payload carries no provider config | `phase2/cells-hyperv.txt` |
| S0 SQL discovery | WIN-A (SQL Server 2025 Express) | 0.112.8 | PASS after D20/D20b — instance `SQLEXPRESS` 17.0.1000.7 returned and persisted to `sql_instances` (`databases: []` because the enrichment hits D22) | `phase2/mssql-instances2.json` |
| S1 SQL on-demand full backup of `AssureDB` | WIN-A | 0.112.8 | D20b cleared (the command now reaches `sqlcmd`); FAIL (D22) — ODBC 18 `sqlcmd` refuses the self-signed server certificate because the helper never passes `-C` | `phase2/cells-sql.txt` |
