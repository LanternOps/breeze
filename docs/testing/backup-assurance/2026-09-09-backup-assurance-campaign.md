...
| S2 SQL restore to `AssureDB_restored` / S3 `RESTORE VERIFYONLY` | WIN-A | 0.112.10 | FAIL (D23b) — `Msg 3201, State 2` on the read side: the helper downloads the `.bak` into SystemTemp and the SQL Server service cannot open it for `RESTORE … FROM DISK` / `RESTORE VERIFYONLY`; fix in flight | `phase2/cells-sql5.txt` |
