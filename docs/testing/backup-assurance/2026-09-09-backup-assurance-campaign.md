...
| R1/R4 GC | LNX (API, `BACKUP_GC_GRACE_MS=1000`) | API `b1f2542c6e` | expired rows `18ce3d7f`, `faf1938b`, `e627b5f5` | FAIL (D17) — cleanup aborted on the first row's FK violation; 0 rows deleted, 0 objects reclaimed; re-run pending the fix | `runs/lnx/postfix-GC.txt`, `postfix-GC2.txt` |
