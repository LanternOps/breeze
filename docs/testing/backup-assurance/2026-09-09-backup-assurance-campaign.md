...
| D23 | HIGH (every SQL Server host where the helper runs as a service) | SQL S1 | `BACKUP DATABASE … TO DISK` targets `os.TempDir()` = `C:\\Windows\\SystemTemp` under SYSTEM; the SQL Server service account cannot open it → `Msg 3201` Access is denied on every backup. `WITH COMPRESSION` is also unsupported on Express (Msg 1844). | FIXED `aa0ccec62f` — target dir = the instance's `InstanceDefaultBackupPath` (`breeze\\` subfolder) with a ProgramData fallback granted to the SQL service account via icacls; `WITH COMPRESSION` skipped on Express and retried once without it on Msg 1844; caller deletes the `.bak` after upload (follow-up commit); [#5459](https://github.com/LanternOps/breeze/issues/5459) |
...
| S1 SQL on-demand full backup of `AssureDB` | WIN-A | 0.112.9 | D20/D20b/D22 cleared end to end — the route answers 202, the queued result is applied to the job with the real SQL error; FAIL (D23) — `Msg 3201` Access is denied: the helper points `BACKUP TO DISK` at `C:\\Windows\\SystemTemp`, unreadable by the SQL Server service account | `phase2/cells-sql.txt` |
