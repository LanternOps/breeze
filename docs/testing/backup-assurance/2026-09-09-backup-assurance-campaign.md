...
| C1 cancel + resume | LNX-QEMU | 0.112.1 | cancelled job `8efe4397` → resumed `snapshot-20260909T190934Z-8f5d8ba7` | PASS (cancel in the scan/dedupe phase) — Stop → `cancelled`, `Cancelled by user`, no snapshot published, checkpoint journal kept, helper alive; the next run logged `resuming interrupted backup from checkpoint journal … resumedBytes=0` and completed 10,048 files with 10,047 referenced and the new 1.5 GiB file uploaded. A cancel timed inside the 1.5 GiB upload is the last take (O24 makes it hard to aim). | `runs/lnx/postfix-C1-job.json`, `postfix-C1b-*.json` |
...
| D16 | HIGH (job stuck forever) | (API restart) ×2 | Two Windows file jobs dispatched while the lab API was restarting stayed `running` for 25+ min with the device online, `transferredSize` = full and `snapshotId` set, no snapshot row, no reaper action. Agent log: `failed to submit command result … submit result failed with status 404` (the API answered 404 for unmatched routes mid-restart); the helper then finished the run (`backup run finished status=completed … filesBackedUp=10045`) and that terminal result was never delivered or logged again; no outbox file exists. An API deploy/restart at the wrong second therefore orphans in-flight backups. Promotes O23. | analysis delegated: agent retry/outbox on 404/5xx for command results, server-side late-result acceptance, reaper rule for "online + idle + running" |
