...
| O23 | MED | (outage) | A Windows file job dispatched during the API outage stayed `running` for 25+ min after the API returned even though the agent was online and had finished — the result sent during the outage was not redelivered and no reaper rule fired. Cancelled by hand. | issue: outbox redelivery after API restart + reaper rule for "device online, helper idle, job running" |
