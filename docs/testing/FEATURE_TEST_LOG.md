...
- Sealed-review web: 7 files / 97 tests passed, including open SSE response handling, every MFA completion boundary, deferred preferences, Web Locks/fallback ordering, caught and uncaught rejection recovery, idempotent caller installation, and cross-operation password/CF/TOTP/recovery/passkey serialization. Full web: 440 files / 3,429 tests passed with bounded workers; the final test-only uncaught-rejection regression then passed in the 60-test auth suite.
...
- Sealed review limits terminal reauthentication inspection to successful JSON responses so event streams/downloads are returned without cloning or draining their bodies; fences TOTP, recovery, and every passkey ceremony boundary; and fences all preference/user/permission/appearance/locale writes. A shared `breeze-auth-session-transition` Web Lock with a rejection-safe in-tab fallback serializes refresh, password, Cloudflare, TOTP/recovery, and whole passkey ceremonies without nested acquisition. Successful sessions install before releasing the transition, so the newer queued intent deterministically becomes current.
