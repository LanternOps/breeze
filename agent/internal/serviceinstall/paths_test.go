...
func TestProtectedBinaryPathIn(t *testing.T) {
	...
	got, err := ProtectedBinaryPathIn(programFiles, "nu-agent.exe")
	if err != nil {
		t.Fatalf("ProtectedBinaryPathIn returned error: %v", err)
	}

	want := filepath.Join(programFiles, "Breeze", "nu-agent.exe")
	if got != want {
...
func TestProtectedBinaryPathInRejectsInvalidInput(t *testing.T) {
	if _, err := ProtectedBinaryPathIn("", "nu-agent.exe"); err == nil {
...
func TestStageProtectedBinaryCopiesExecutableToTarget(t *testing.T) {
	dir := t.TempDir()
	source := filepath.Join(dir, "Downloads", "nu-agent.exe")
	target := filepath.Join(dir, "Program Files", "Breeze", "nu-agent.exe")
...
func TestStageProtectedBinaryKeepsProtectedExecutable(t *testing.T) {
	target := filepath.Join(t.TempDir(), "Program Files", "Breeze", "nu-agent.exe")
