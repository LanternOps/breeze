...
var remainingVendorStates = func() map[string]map[string]string {
	rows := []struct{ key, raw, normalized string }{
		...
		{"arcconf/physical_disk", "Online|Hot Spare|Ready|Failed|Rebuilding|Raw (Pass Through)|Global Hot-Spare|Dedicated Hot-Spare", "online|hotspare|ready|failed|rebuilding|jbod|hotspare|hotspare"},
		{"arcconf/cache_battery", "Optimal|Charging|Not Installed|Failed|ZMM Optimal", "ok|charging|missing|failed|ok"},
		...
		{"zfs/physical_disk", "ONLINE|DEGRADED|FAULTED|OFFLINE|UNAVAIL|REMOVED|CANT_OPEN|AVAIL|INUSE", "online|degraded|failed|offline|missing|missing|missing|hotspare|online"},
	}
