...
func ClassifyAsset(host DiscoveredHost) (string, string, string) {
	...
	case strings.Contains(sysDescr, "router") || strings.Contains(sysObjectID, "router"):
		assetType = "router"
	case strings.Contains(sysDescr, "firewall"):
		assetType = "firewall"
	case strings.Contains(sysDescr, "access") && strings.Contains(sysDescr, "point"):
		assetType = "access_point"
	case hasPort(host.OpenPorts, 3389) || hasPort(host.OpenPorts, 445):
		assetType = "workstation"
	case hasPort(host.OpenPorts, 22) && hasPort(host.OpenPorts, 80):
		assetType = "server"
